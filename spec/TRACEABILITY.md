...
| TM-FUNC-012 | partial | P3-D.0 typed schema-v1 settings/store/portable preview over the crate-private redundant-record core, library-only `tokenmaster-state`, controlled `tokenmaster-platform` durable files, and future package/app integration | Tasks 1-4 stable error/bound/authority, exact-child staged move/replace, strict A/B envelope, safe current/fallback/default settings, portable/device separation, stale/idempotent preview commit, reread target, and process-death contracts pass; package/snapshot/retention/recovery/safe-mode evidence pending |
...
| TM-PERF-004 | partial | Task 3 two-pass bounded record writer plus future P3-D.0 capacity-one maintenance worker and package codecs | record save retains no full encoded JSON buffer and writes at most 256 KiB per call; 10,000-trigger, package-codec, latency, sleep/resume, and repeated memory/handle/thread/USER/GDI gates pending |
...
| TM-DATA-011 | partial | controlled exact-child durable files, crate-private redundant records, strict typed settings, plus future fixed packages, self-describing restore points, and recovery journal | strict 64-byte-header/40-byte-footer A/B records, 1 MiB cap, two-pass streaming save, highest-valid/fallback/equal-conflict rules, exact schema-v1 portable/device classes, bounded candidates, generation/digest target verification, three state process-death phases, prior 60 replacement kill rounds, and 60 inactive-slot kill/race rounds pass; package vectors, retention, journal transitions, quarantine, and compatibility gates pending |
...
| TM-SEC-008 | partial | library-only state authority boundary, crate-private six-child record authority, fixed-purpose typed settings, platform-owned exact-child durable staging/replacement, and future strict package/quarantine | 34-mutation authority gate plus link/reparse/type rejection, fixed record names, 1 MiB record/import and 256 KiB call bounds, deny-unknown versioned settings, valid-envelope newer-version write protection, bounded sequence decode, forbidden-field canaries, portable/device isolation, redaction, equal-generation conflict, exact old-target backup, preserved recovery artifacts, post-publication `RecoveryRequired`, and both backed-up/redundant Windows kill suites pass; package/recovery/adversarial gates pending |
