...
| TM-FUNC-008 | partial | built-in provider-neutral drafts plus constant-state coordinator, bounded synchronous ports, and one-shot executor; worker, live Codex composition, and plugin host pending | provider/Codex/accounting contracts plus P1-C.1 coordinator, P1-C.2 ports, and P1-C.3 one-shot phase/failure contracts |
...
| TM-PERF-001 | partial | parser, reader, domain, store bounds, P0-E composition, and P1-C coordinator/executor | 300-file/300-event, streamed discovery with no source list, 256 observation/relation batches, 256 replay-source pages, 32-KiB checkpoints, 18 chunk updates, 4,096 continuation cap, reopen, and 10,000-hint constant-state contracts pass; query/plugin runtime bounds pending |
...
| TM-DATA-006 | partial | reader/parser/store plus engine value/batch/page/execution limits | line/resume, 32-KiB opaque checkpoint, 256 batch/page, 18 chunk-update, 4,096 continuation, fixed-result, and SQLite-i64 counter bounds; full UI/query/plugin limits pending |
...
| TM-SEC-003 | implemented | provider/Codex/store/engine errors and value types | serialized/debug privacy, path-redaction, sealed identity, path-substitution, and raw-archive-write compile-fail contracts |
| TM-SEC-004 | partial | transactional scoped scan/replay authority, bounded pruning, CAS/preparation, immutable legacy, exact seal/carry-forward/discard, and one-shot fail-closed composition | P1-B proves archive authority; P1-C.3 adds cross-scope rejection, replay-handle continuity, all-phase cancellation/deadline, exact cleanup, and no-publication failure contracts; live Codex runtime remains |
...
cancellation/deadline coverage and exact replay cleanup; the worker shell remains
P1-C.4.
