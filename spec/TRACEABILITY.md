...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, P3-A-P3-B live shell, P3-C Dashboard, P3-D.1 History, complete P3-D.2 Sessions, P3-D.3 Models, P3-D.4 Projects, P3-D.5 Activity, and P3-D.6 Notifications | prior evidence plus responsive bounded Models/Projects/Activity/Notifications, explicit evidence/truncation truth, and in-place query-free switching with no extra analytics/Git/Activity/benefit query pass; Help, visible notification delivery/settings, and P3-E shell integration remain |
...
| TM-FUNC-010 | partial | prior benefit/reminder stack plus explicit all-current overview, P3-C banked-reset summary, and P3-D.6 expiry-safety center | separate reset/credit/temporary/unavailable lots, five expiry precisions, effective inherited/override profiles, up to eight leads, nearest expiry/due, coverage, evidence, warnings, and bounded 32-scope/256-lot rendering pass; presentation receipts, settings synchronization/editing, snooze, quiet hours, OS delivery, and activation remain |
...
| TM-UI-001 | partial | P3-A shell, P3-C responsive Dashboard, P3-D.1 History, P3-D.2 Sessions list/detail, P3-D.3 Models, P3-D.4 Projects, P3-D.5 Activity, and P3-D.6 Notifications | prior evidence plus wide/narrow bounded reminder profiles and separate current lots retaining policy, expiry, evidence, warning, and accessible row truth pass; Help and full accessibility acceptance remain P3-D-P4 |
| TM-UI-002 | partial | immutable runtime/query/product publication plus bounded Dashboard/History/Sessions/Models/Projects/Activity/Notifications route-only Slint application | epoch/newer-generation replacement includes capped History, Sessions list/detail, Models, Projects, Activity, reminder-profile, and benefit-lot models; route-only updates perform no query/model rebuild/window recreation/timer/animation/delivery acknowledgement; hot skin/locale/layout settings remain P4 |
| TM-PERF-001 | partial | bounded backend, one-current product/Desktop projection, bounded query/operation workers, one snapshot/runtime-observation/event gate, and capped Dashboard/History/Sessions/Models/Projects/Activity/Notifications models | prior gates plus 32 reminder scopes, 256 current lots, eight leads/profile, one existing overview, replace-only models, zero route-time work/polling, and 10,000-replacement release pass; measured visible-paint and long-run release evidence remain P4/P6 |
...
| TM-PERF-003 | partial | immutable engine/product publication, indexed facades, epoch/stale-safe desktop projection, bounded query controller, newest-only bridge, and live app hints | prior evidence plus one shared exact recent-days History/Models/Projects envelope, one existing latest Activity page, no extra query, sequential Sessions page, and latest-only exact detail reads through the existing worker pass; P4 paint and final release evidence remain |
...
| TM-DATA-001 | partial | prior privacy boundaries plus path-private app root and bounded Dashboard/History/Sessions/Models/Projects/Activity/Notifications models | Notifications adds only ordinals, provider-neutral lot/profile/expiry/evidence truth; no provider/account/workspace/scope/lot/delivery/window identity, path, content, credential, receipt, or authority crosses Slint; remaining P3-D-P5 surfaces repeat gates |
...
| TM-DATA-006 | partial | prior bounded publication plus current Dashboard/History/Sessions/Models/Projects/Activity/Notifications projections | prior caps plus 32 profile rows, 256 separate lots, eight leads/profile, five typed expiry precisions, explicit frontend truncation, and no prior model/query/cache pass; later pagination, rhythm aggregates, and P4 switch limits remain |
...
| TM-SEC-002 | partial | prior strict boundaries plus separate app/frontend authority and privacy-bounded Dashboard/History/Sessions/Models/Projects/Activity/Notifications/reliable-state UI | Notifications has one mapping and two model-application sites over the existing overview, zero delivery take/ack/release or activation control, and zero provider/account/workspace/scope/lot/delivery/window/path/content/receipt authority; remaining P3-D-P5/plugin and interactive accessibility suites remain |
