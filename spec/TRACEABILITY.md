...
| TM-FUNC-003 | partial | domain/accounting/Codex parser, schema-v9 price facts, indexed aggregate query facade, P3-C Dashboard, and P3-D.1 History projection | exact token/cost overview, daily-series, breakdown, session page/detail mapping plus visible today and exact recent-30-day range/timezone/evidence/daily detail pass; interactive ranges, remaining exploration, and CLI/MCP remain |
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, P3-A-P3-B live shell, P3-C Dashboard, and P3-D.1 History route | prior Dashboard evidence plus independent bounded History section, responsive real view, and in-place query-free route switching pass; Sessions/Models/Projects/Activity/Notifications/Help and P3-E shell integration remain |
...
| TM-UI-001 | partial | P3-A shell, P3-C responsive Dashboard, and P3-D.1 responsive History view | prior Dashboard evidence plus exact range/timezone/evidence, 30-day trend, wide token-bucket table, narrow summary table, unavailable truth, and accessible row meaning pass; remaining routes and full accessibility acceptance remain P3-D-P4 |
| TM-UI-002 | partial | immutable runtime/query/product publication plus bounded Dashboard/History route-only Slint application | accepted-generation replacement now includes one capped History model; route-only updates still perform no query/model rebuild/window recreation/timer/animation; hot skin/locale/layout settings remain P4 |
| TM-PERF-001 | partial | bounded backend, one-current product/Desktop projection, bounded query/operation workers, one snapshot/runtime-observation/event gate, and capped Dashboard/History lists | prior burst gates plus fixed 30-row History request/projection/model, one sequential existing-worker query, and route-only hot path pass; measured visible-paint and long-run release evidence remain P4/P6 |
...
| TM-DATA-001 | partial | prior privacy boundaries plus path-private app root and identity-free Dashboard/History models | History adds only civil dates and aggregate values; projection/UI contracts retain no raw session/event/account/workspace/project/source identity; remaining P3-D-P5 surfaces must repeat gates |
...
| TM-DATA-006 | partial | prior bounded publication plus current Dashboard and independent History projection/model | prior exact Dashboard caps plus one 30-row newest-first History model, exact recent-days range, and no prior-range cache pass; paged Sessions and other P3-D views plus P4 switch limits remain |
...
| TM-SEC-002 | partial | prior strict boundaries plus separate app/frontend authority and identity-free Dashboard/History/reliable-state UI | History adds no path/ID/cursor/SQL/content authority and uses the same worker/snapshot/event surfaces; remaining P3-D-P5/plugin and interactive accessibility suites remain |
