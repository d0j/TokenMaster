...
| TM-FUNC-004 | partial | exact joined product status, immutable product reducer, fixed route readiness, and the P3-A production Slint shell; route payloads remain | P2-F contracts plus exact 11-row product-to-desktop mapping, stable state/reason rendering, callback selection, release build, and desktop authority audit pass; P3-B-P3-E remain |
...
| TM-UI-002 | partial | immutable runtime/query/product publication plus P3-A one-current-projection desktop state | prior generation contracts plus fixed snapshot mapping, stable selection, equal/older UI-generation rejection, and one bounded Slint model replacement pass; P3-B worker publication and P4 switches remain |
| TM-PERF-001 | partial | bounded backend plus one-current product and desktop projections | prior gates plus P3-A fixed 11-row model/no worker/timer/cache/history audit; UI 10,000-switch and long-run release evidence remain P4/P6 |
...
| TM-PERF-003 | partial | immutable engine/product publication, indexed facades, and P3-A stale-safe desktop projection | prior scale evidence plus exact product-generation rejection at the UI boundary; P3-B controller and P4 visible-paint evidence remain |
...
| TM-DATA-006 | partial | bounded backend, one-current product snapshot, and one-current desktop route model | prior limits plus fixed 11-row/11-reason desktop projection, no mock model/history/worker/timer/cache, and route-source audit pass; paged route payload and P4 switch limits remain |
