...
| --- | --- | --- | --- |
| TM-FUNC-001 | implemented | `crates/provider`, Codex roots/files | provider, discovery, and 16 enumeration contracts; normal bounded directory traversal prefers newest date-shaped entries for faster first-use publication and streams directories above the fixed sort cap |
| TM-FUNC-002 | implemented | Codex reader plus store/runtime incremental path | zero-payload unchanged, exact tail bytes, multi-batch restart, new/missing source, replacement/truncation/profile-scope and malformed-input recovery contracts; descriptor-local SHA-256 continuation removes repeated partial-chunk prefix reads while identity/length/mtime invalidation and bounded restart reconstruction preserve exact proofs; strict-superset staging extends in place, replacement publication remains private until atomic promotion, and cancellation is observed between bounded replay transactions |
| TM-FUNC-003 | partial | domain/accounting/Codex parser, schema-v9 price facts, indexed aggregate query facade, WMT-quality bounded Dashboard, P3-D.1 History, P3-D.2 Sessions list/detail/later-page navigation, P3-D.3 Models, P3-D.4 Projects, P3-D.5 Recent activity, and bounded rhythm rollups | Dashboard exposes exact count-only cold-import progress, a dominant accessible Today cost metric, and the existing recent daily token/cost series as axes, dual lines, sparse markers, and hover detail; P3-D interactive ranges and full rhythm cover fixed 1/7/30 envelopes plus 24 hourly and seven Monday-Sunday buckets, capped at 30 civil days/768 occurrences/2,304 segments with DST/fractional/skipped-date truth; shell parity, time-to-first-useful-data, CLI/MCP, and remaining parity remain |
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, P3-E.1 route command palette, P3-E.2 compact quota mode, P3-E.3 typed tray lifecycle, P3-E.4 current-session activation, P3-E.5 current-user startup, app-owned in-app expiry presentation, global reminder settings editor, P4-E layouts, P4-F board preferences, P4-G unified localization, and a locally accepted WMT-inspired fixed-header/footer release shell with real Partial data/scroll evidence | P4-G closes the en/ru/pseudo locale axis and hot reprojection; per-scope editing, reminder OS/tray delivery, usage alerts, package/signing, and external presentation acceptance remain |
| TM-FUNC-005 | partial | separate `crates/desktop` frontend, sole `crates/app` production binary, `tokenmaster-platform` current-session/current-user startup owners, and P4-E/P4-F/P4-G complete presentation owner | the real Slint shell is shown before bounded archive prepare/live-start workers complete and accepts the prepared live bundle in place; P4-G persists schema-v7 locale and proves hot en/ru/pseudo reprojection across Slint/Rust labels while retaining stable payloads. Typography/row sizing, accessibility/DPI/paint/resource verification, and live Windows acceptance remain P4/P6 |
...
| TM-FUNC-007 | partial | accounting lineage/classifier, scalable replay archive, P0-E composition, P1-A retention, active Partial engine publication, and bounded exact-replay batching | synthetic JSONL baseline/append/restart/replay/quality/atomic-replacement/truncation-retention/failure contracts pass; exact parent-index seeks, bounded statement caching, disposition-only classification refresh, schema-v14 full-key time-rollup maintenance, schema-v15 removal of the unused write-heavy child index, bounded multi-command commits, root-session fast path, conditional exact-row verification, and one-batch prefetch preserve exact authority; runtime snapshots expose durable Partial truth before worker completion. The final real 4,008-source receipt reached 435 sources and 265,386 observations in about 122 seconds at 34.6 MiB, while complete cold-import and cross-product speed evidence remain open |
...
| TM-FUNC-010 | partial | prior benefit/reminder stack, expiry-safety center, app-owned in-app presentation receipts, and global reminder settings synchronization | portable settings map generation `N` to global revision `N + 1`; inherited due rebuild preserves overrides and delivery/ack/provider evidence, aggregate results cover valid overridden scopes without post-commit fallibility, startup Busy/unavailable keeps the exact policy Pending while runtime health degrades independently, and the next successful synchronization makes one same-session restart attempt only for a missing StoreUnavailable reminder runtime; snooze, quiet hours, OS delivery, and activation remain |
...
| TM-UI-001 | partial | complete bounded data routes, static Help/About, P3-E.1 command palette, P3-E.2 compact quota mode, P3-E.3 tray lifecycle, P3-E.4 Show/restore/focus activation, P3-E.5 startup-status/actions, transient accessible expiry panel, P4-G locale projections, first-safe-import Dashboard status, and bounded route viewports | compiled Slint proves palette/compact activation, a truthful Partial/RecoveryPending in-main-window loading state while safe data remains visible, en/ru/pseudo labels, and working scroll; a software-pixel contract and native exact-package capture prove a non-black/non-uniform local frame, while authenticated disposable-host focus/accessibility/sign-in and interactive acceptance remain P4/P6 |
...
| TM-PERF-001 | partial | bounded backend, one-current projections, capped P3-E.1/P3-E.2 models, queue-free P3-E.3 lifecycle router, one P3-E.4 native thread, one notification receipt worker, constant-capacity bridges, bounded staging replay, retained query-resource warm-up, first-safe-import status, and descriptor-local proof continuation | a reproduced reader benchmark removed 16,060,240 repeated prefix bytes from a 1,038,170-byte/32-batch source and reduced optimized reader time from 23 to 15 ms with identical checkpoint/full-prefix proof. A real pre-fix run exposed safe Partial data with bounded memory but extreme logical reads; modified-app completion and cross-product evidence remain open |
| TM-PERF-002 | partial | software renderer, native packaged-window capture, and M0 resource gates | the software-pixel startup frame and a 1,133 by 756 native exact-package capture are non-black/non-uniform; authenticated disposable-host DPI/accessibility/interaction, uninterrupted soak, and external receipts remain absent |
| TM-PERF-003 | partial | immutable publication, bounded query controller, newest-only snapshot bridge, capacity-one Sessions terminal rollback, independent notification epoch bridge, durable-progress recovery hints, live app hints, and active Partial generation observation | current replay keeps exact indexed and bounded transactions; the engine snapshot no longer waits for terminal worker completion before exposing a newer durable Partial generation. Real-history evidence still identifies durable per-observation replay storage as the open cold-import bottleneck; bulk-ingest, final package, and external release evidence remain |
...
| TM-REL-001 | partial | M0 scripts, conditional standard PSGallery bootstrap, serial Windows GNU Cargo policy, 75-minute M0 CI receipt budget, fixed stage-progress labels, latest-only terminal-completion, active-range and active-detail admission, quota-startup submission rendezvous, contract-correct reminder startup Pending correlation and exact-child same-session StoreUnavailable recovery, CRLF-safe pre-commit receipt guard, durable Git sibling-isolation assertion, and isolated current-session resource contracts, deterministic product producer/validator, product receipt schemas, separate P3-D.0 developer receipt, and `P3E_ACCEPTANCE.md` | the P6 unsigned producer and P3-E preflight bind one clean commit, closed package/manifest, producer receipt, packaged/tested executable, and operator receipt; M0 restores the standard named PSGallery repository only when absent before installing its pinned Pester, serializes GNU linker work, grants the measured 75-minute clean-run wall time once, emits only fixed begin/pass labels for each existing stage so a cancellation identifies the active stage without exposing command details, and requires a later timeout to split the receipt path rather than raise its budget. It correctly waits for terminal worker state rather than a count of lossy latest receipts, holds a range worker through its `busy` page-admission assertion before unwind-safe release, observes an active bounded detail before asserting its inverse range `busy`, waits for the completed quota startup outcome together with its scheduler submission count, accepts only the specified Synchronized reminder state or the specified Pending/no-owner/StoreUnavailable startup contention state, restarts a startup-contention reminder runtime only after a successful later synchronization in an exact child test process, normalizes only its self-included M0 guard source before testing conversion-before-final-commit order, proves a valid Git sibling as durable Complete output instead of exact cumulative scheduler counts, and measures 4,096 current-session owner cycles only in an exact child test process without widening its native-resource bounds. Authenticated external receipts, current schema-4 P3-D.0 exact-commit receipt, signing, and final product acceptance remain pending |
...
| TM-DATA-004 | implemented | scoped scan/rebuild plus replay-aware current publication, paired-CAS tail facts, exact admission, durable partial/recovery, retained promotion/discard | atomic faults, stale CAS, unchanged/append/multi-batch/new/missing/restart/deadline/rebuild contracts pass; a valid staging replacement is preserved across restart, rebound only to an exact new scan, and promotes over current atomically; a queued session-work row cannot block a later manifest member, proven no-op EOF closes its source, and caught-up resume repair is exact except for opaque resume bytes |
| TM-DATA-005 | implemented | writable usage/quota/benefit/Git store plus separate `UsageReadStore` | strict schema v15 retains all prior facts, atomically replaces four broad time-rollup mutation triggers with full-primary-key seeks, drops one unused write-heavy replay index through an exact rollback-tested v14 migration, and preserves one exact scalar cross-family product-status transaction with defensive deadline/corruption/progress cleanup and no historical scan |
...
| TM-DATA-007 | partial | replay facts/classifier in a private overlay plus schema-v4 self-contained canonical projection with deterministic selection/retention | v1/v2/v3-to-v4 migration plus replay/append/restart/300-file/atomic-replacement/truncation truth-table/failure contracts pass; transactional count-only progress, strict-superset staging extension, exact-index replay, restart-safe private replacement, and atomic promotion pass; the real large-import progress/shutdown receipt is green, while complete cold-import acceptance remains open |
...
| TM-DATA-009 | partial | typed benefit inventory, schema-v12 due/outbox/ack, durable reminder runtime, app-owned visible presentation, and global profile projection | global profile replacement is immediate and inherited-only; returned counts are validated before commit and desired settings survive startup Busy/unavailable as exact retryable Pending independently of optional runtime health, while the next successful synchronization may make one bounded same-session restart of the missing StoreUnavailable runtime; OS delivery and activation intents/receipts remain |
