...
| TM-FUNC-012 | partial | P3-D.0 typed settings plus store-owned verified SQLite candidates over crate-private records and controlled durable files; future package/app integration | Tasks 1-5 stable authority/settings evidence plus WAL-consistent Online Backup, defensive independent verification, compaction, identity binding, and cleanup recovery pass; package/retention/recovery/safe-mode evidence pending |
...
| TM-PERF-004 | partial | bounded record writer and Task 5 page-stepped/streaming snapshot verifier plus future capacity-one maintenance worker and package codecs | record save writes at most 256 KiB per call; snapshot uses 64-page steps, fixed hash buffers, SQLite limits, bounded schema memory and cancellation/deadline; package-codec, latency, sleep/resume, and repeated resource gates pending |
...
| TM-DATA-011 | partial | controlled durable files, redundant records, typed settings, and verified standalone SQLite candidates plus future fixed packages/catalog/journal | prior evidence plus fixed snapshot/compact namespaces, Online Backup WAL truth, exact version/integrity/FK/schema/semantic gates, physical identity/length/SHA-256 binding, and bounded cleanup recovery pass; package vectors, retention, journal transitions, quarantine, and compatibility gates pending |
...
| TM-SEC-008 | partial | prior state/platform authority plus fixed store snapshot capabilities and future strict package/quarantine | prior evidence plus no main-only backup, no caller SQL/name/path, defensive/query-only verifier, trusted-schema/DQS off, cell checks, mmap zero, explicit SQLite limits, stale-identity rejection, typed transient/corruption separation, private errors, and final independent Task 5 review pass; package/recovery gates pending |
