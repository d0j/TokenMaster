...
| TM-FUNC-003 | partial | domain/accounting/Codex parser, schema-v9 price facts, indexed aggregate query facade, P3-C Dashboard, P3-D.1 History, and P3-D.2 Sessions list/detail | exact token/cost overview, daily-series, breakdown, session page/detail query mapping plus visible today, exact recent-30-day History, newest 64-row Sessions page, and generation-safe exact 32+32 detail pass; ranges, remaining exploration, and CLI/MCP remain |
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, P3-A-P3-B live shell, P3-C Dashboard, P3-D.1 History, and complete P3-D.2 Sessions route | prior Dashboard/History evidence plus independent bounded Sessions section, responsive real list/detail, explicit page/truncation/evidence truth, and in-place query-free route switching pass; Models/Projects/Activity/Notifications/Help and P3-E shell integration remain |
...
| TM-UI-001 | partial | P3-A shell, P3-C responsive Dashboard, P3-D.1 responsive History, and P3-D.2 responsive Sessions list/detail | prior evidence plus exact range/timezone/evidence, 30-day History trend/table, wide/narrow 64-row Sessions table, keyboard/focus/hover selection, and explicit bounded detail states/summary/breakdowns pass; remaining routes and full accessibility acceptance remain P3-D-P4 |
| TM-UI-002 | partial | immutable runtime/query/product publication plus bounded Dashboard/History/Sessions route-only Slint application | epoch/newer-generation replacement includes one capped History, one capped Sessions list, and one 32+32 detail model; selection updates locally then uses one typed existing-worker intent; route-only updates perform no query/model rebuild/window recreation/timer/animation; hot skin/locale/layout settings remain P4 |
| TM-PERF-001 | partial | bounded backend, one-current product/Desktop projection, bounded query/operation workers, one snapshot/runtime-observation/event gate, and capped Dashboard/History/Sessions models | prior burst gates plus fixed 30-row History, 64-row Sessions list, one latest-only detail slot, 32+32 detail rows, sequential existing-worker queries, and route-only hot paths pass; measured visible-paint and long-run release evidence remain P4/P6 |
...
| TM-PERF-003 | partial | immutable engine/product publication, indexed facades, epoch/stale-safe desktop projection, bounded query controller, newest-only bridge, and live app hints | prior evidence plus exact recent-days DST/range contracts and sequential off-UI History, Sessions page, and latest-only exact detail reads through the existing worker pass; P4 paint and final release evidence remain |
...
| TM-DATA-001 | partial | prior privacy boundaries plus path-private app root and bounded Dashboard/History/Sessions models | History adds only civil dates and aggregates; Sessions list adds aggregate timestamps/counts/tokens/cost/`has_more`; detail adds only exact aggregate summary/evidence and approved path-free model/project aliases. No raw session/event/account/workspace/source identity, opaque key, cursor, path, or content crosses Slint; remaining P3-D-P5 surfaces must repeat gates |
...
| TM-DATA-006 | partial | prior bounded publication plus current Dashboard and independent History/Sessions projections/models | prior exact Dashboard caps plus one 30-row History model, one 64-row Sessions list, one 32-model+32-project detail model, explicit continuation/truncation, three-axis latest-only correlation, and no prior-page/detail cache pass; later pagination, other P3-D views, and P4 switch limits remain |
...
| TM-SEC-002 | partial | prior strict boundaries plus separate app/frontend authority and privacy-bounded Dashboard/History/Sessions/reliable-state UI | exact detail resolves ordinal to opaque key only inside the existing controller worker; epoch/product/selection generations reject stale work, safe mode rejects admission, and Slint receives no path/key/cursor/SQL/content authority; remaining P3-D-P5/plugin and interactive accessibility suites remain |
