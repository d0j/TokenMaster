...
| TM-FUNC-012 | partial | prior reliable-state stack plus completed Task 12 application lifecycle, Task 14 sealed file selection, and Task 15 Data & Recovery UI | prior evidence plus path-free UI config/backup/verify/restore/rebuild/policy intents, exact reviewed restore identity, actual-start operation phases, durable restore/reconstruction receipt and restart-safe mandatory reconciliation, typed unavailable metrics, bounded Data Health/Settings views, and application/desktop policy suites pass; Task 16-18 fault/resource/acceptance closure remains |
...
| TM-PERF-001 | partial | bounded backend, one-current product/Desktop projection, bounded query/operation workers, one snapshot/runtime-observation/event gate, and capped Dashboard lists | 10,000 snapshot and operation hints retain latest/current plus one follow-up only; 32/32/240/12/8/12/32 UI caps, one model replacement per list, and route-only hot path pass; measured visible-paint and long-run release evidence remain P4/P6 |
...
| TM-PERF-004 | partial | bounded reliable-state path, one joined constant-state app worker, immutable catalog/UI projections, and event-driven reconstruction barrier | prior fixed bounds plus one operation worker/thread, capacity-one wake/latest completion, one command active/follow-up under 10,000 hints, off-UI file/SQLite/compression/recovery work, 2 MiB config ceiling, bounded staging/quarantine, short-held catalog mutex, atomic waits, and Windows maintenance resource gates pass; Task 17 full operation/UI latency and repeated recovery resource receipts remain |
...
| TM-DATA-011 | partial | complete reliable-state data/application/UI contour through Task 15 | prior evidence plus optional journal backup identity for explicit source reconstruction, fresh-schema staged/reverified atomic promotion, bounded corrupt-set quarantine, mandatory completed recovery refresh before healthy publication and after restart, retry without repeated reconstruction, latest-only path-free operation/state projections, and explicit typed unavailable metrics/domains pass; Task 16-18 fault/resource/acceptance closure remains |
...
| TM-SEC-002 | partial | prior strict boundaries plus separate app/frontend authority, identity-free Dashboard, and path-free reliable-state UI | desktop audits reject storage/runtime/platform authority, arbitrary path callbacks, UI polling/progress queues, private IDs, hidden reconstruction loss, and extra worker/snapshot/event surfaces; remaining P3-D-P5/plugin and interactive accessibility suites remain |
...
| TM-SEC-008 | partial | sealed reliable-state layers plus complete application/UI command and reconstruction contour through Task 15 | prior evidence plus exact irreversible-phase cancellation closure, reviewed restore identity pinning, typed dialog-to-worker capabilities, no-backup reconstruction only after definitive corruption/no verified point, normal-schema full verification, journaled quarantine/promotion/resume, restart-safe mandatory source reconciliation before healthy maintenance, and explicit non-reconstructible-loss UI receipt; Unix cleanup hardening plus Task 16-18 adversarial/resource/release gates remain |
