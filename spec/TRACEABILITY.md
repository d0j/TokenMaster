...
| TM-FUNC-012 | partial | P3-D.0 crate-private redundant-record core, library-only `tokenmaster-state` foundation, controlled `tokenmaster-platform` durable files, and future typed store/app integration | Tasks 1-3 stable error/bound/authority, exact-child staged move/replace, strict A/B envelope, corruption fallback/conflict, post-publication recovery, and process-death contracts pass; settings/package/snapshot/retention/recovery/safe-mode evidence pending |
...
| TM-DATA-011 | partial | controlled exact-child durable files, crate-private redundant records, plus future fixed typed packages, self-describing restore points, and recovery journal | strict 64-byte-header/40-byte-footer A/B records, 1 MiB cap, two-pass streaming save, highest-valid/fallback/equal-conflict rules, post-commit recovery classification, three state process-death phases, prior 60 replacement kill rounds, and 60 inactive-slot kill/race rounds pass; typed settings, package vectors, retention, journal transitions, quarantine, and compatibility gates pending |
...
| TM-SEC-008 | partial | library-only state authority boundary, crate-private six-child record authority, platform-owned exact-child durable staging/replacement, and future strict package/quarantine | 33-mutation authority gate plus link/reparse/type rejection, fixed record names, 1 MiB record/256 KiB call bounds, redaction, equal-generation conflict, exact old-target backup, preserved recovery artifacts, post-publication `RecoveryRequired`, and both backed-up/redundant Windows kill suites pass; package/recovery/adversarial gates pending |
