...
| TM-FUNC-002 | implemented | Codex reader plus store/runtime incremental path | zero-payload unchanged, exact tail bytes, multi-batch restart, new/missing source, replacement/truncation/profile-scope and malformed-input recovery contracts; a known later partial-replay source remains admissible while session work is queued, and invalid `cwd` clears only the transient repository hint and leaves valid usage admissible |
...
| TM-FUNC-007 | partial | accounting lineage/classifier, scalable replay archive, P0-E composition, and P1-A retention | synthetic JSONL baseline/append/restart/replay/quality/atomic-replacement/truncation-retention/failure contracts pass; a valid replacement staging replay now survives restart, is accepted only after an exact new source scan, and atomically replaces current only after seal/promotion; real large-import throughput and local acceptance remain open |
...
| TM-PERF-003 | partial | immutable publication, bounded query controller, newest-only snapshot bridge, capacity-one Sessions terminal rollback, independent notification epoch bridge, durable-progress recovery hints, and live app hints | current replay no longer performs a global foreign-key scan per append; a clean 3,998-source run advanced 33 to 162 sources in 79 seconds at 82.2 MiB working set with an 8.3 MiB WAL, while failed initial work schedules Recovery only after an archive-generation advance; notification callbacks retain their prior bounded release/re-pump/join guarantees; final package/external release evidence remains |
...
| TM-DATA-007 | partial | replay facts/classifier in a private overlay plus schema-v4 self-contained canonical projection with deterministic selection/retention | v1/v2/v3-to-v4 migration plus replay/append/restart/300-file/atomic-replacement/truncation truth-table/failure contracts pass; transactional count-only import progress is bounded and consistency-checked, and injected first-batch failure proves immediate generation-gated Recovery to Complete; replacement staging remains private until exact-scan resume and atomic promotion; complete large-import acceptance remains open |
