...
| TM-FUNC-009 | partial | exact quota domain values plus planned immutable provider quota epochs and full-reset transitions | bounded IDs, ppm ratios, optional units, provider thresholds, definitions/samples, reset evidence, validated serde, and redacted observation-ID contracts pass; detector/schema/scheduled/early/repeated/reset+allowance/restart/query/UI fixtures pending |
...
| TM-PERF-003 | partial | immutable engine publication, P2-A activity query, schema-v9 transactional token/price aggregates, and immutable cost facade | current/legacy P2-C million gates: rebuild 8,737/8,129 events/s, page p95 376.824/406.604 ms, amplification 1.862x/2.010x, cached overview p95 2.040/2.065 ms, 400-point/four-breakdown p95 148.168/156.080 ms, all-32-scope 158.588/162.504 ms, session page p95 below 14 ms and detail below 1 ms; isolated single-thread retained-return resource windows preserve 1/2 MiB and structural bounds; P3 UI remains |
...
| TM-DATA-008 | partial | exact normalized quota definitions/samples plus planned epochs, reset and allowance transitions | domain constructors reject unbounded IDs, invalid ppm/time/capacity/evidence relationships, empty facts, non-fixed thresholds, serde bypass, and unknown nested fields; detector/schema/retention/read contracts pending |
...
| TM-SEC-002 | partial | current JSONL/store boundaries plus exact quota domain values validate types and sizes | quota IDs/ratios/units/definitions/samples reject invalid and ambiguous serde input; future provider/config/CLI/MCP/plugin boundary suites pending |
| TM-SEC-003 | implemented | provider/Codex/store/engine errors, value types, redacted worker panic boundary, and redacted quota observation IDs | serialized/debug privacy, path-redaction, sealed identity, path-substitution, raw-archive-write compile-fail, quota identity privacy, fixed panic/fault completion, and panic-strategy compile guard contracts |
...
P2-D Task 1 exact quota domain values is implemented; detector, schema, history,
query, transport, and reset inventory remain. P3 UI evidence remains separate.
M0 interactive/
