...
| TM-FUNC-011 | partial | strict Git domain values, bounded parser/core, exact native read-only backend, and transient Codex repository activity hints | Tasks 1-4 domain/aggregate/process/synthetic/privacy contracts pass; schema-v13 projection, query facade, runtime scheduling/resource evidence, joined status, and UI remain |
...
| TM-DATA-005 | implemented | writable usage/quota/benefit store plus separate `UsageReadStore` | strict schema v12 retaining dataset/token/price/quota contracts plus benefit-owned objects and durable acknowledgement, exact v8/v9/v10/v11 rollback migrations, read-only query-only defensive 4-MiB/no-migration/no-checkpoint policy, and fixed quota/benefit current/history captures pass |
...
| TM-DATA-010 | partial | non-serializable repository activity hints and bounded opaque Git values; durable schema-v13 projection pending | local-directory namespace/reparse policy, latest-only parser association, old-resume compatibility, runtime side-channel transport, Debug/checkpoint/SQLite privacy, and exact identity/time contracts pass |
