...
| TM-FUNC-012 | partial | P3-D.0 typed settings, store-owned verified SQLite candidates, fixed typed packages, manual age protection, bounded retention, and capacity-one maintenance over sealed capabilities | Tasks 1-9 pass stable authority/settings, WAL-consistent Online Backup, deterministic `.tmconfig`/`.tmbackup`, age v1 export/import, exact prepublication verification, 32-slot catalog/retention, typed candidate streaming, mandatory guards, linked cancellation, healthy-restart seeding, internal-only retry, periodic-disable race, publication-state, lifecycle, and resource gates; recovery/safe-mode evidence pending |
...
| TM-PERF-004 | partial | bounded record/snapshot/package path plus capacity-one native backup maintenance runtime | prior fixed buffers/bounds plus one worker, one scheduler/shared timer, one active/one merged follow-up under 10,000 hints, resume/rollback catch-up, joined lifecycle, and every post-warm-up sample/final point bounded across the 36-cycle Windows success/failure/cancel resource gate; application-composed backup latency and restore-cycle evidence remain |
...
| TM-DATA-011 | partial | controlled files/records/settings, verified SQLite candidates, typed v1 packages, catalog/retention, and constant-state maintenance plus future journal | prior evidence plus exact package vectors, six backward-compatible purposes, sealed typed store-reader bridge, physical/length/SHA revalidation, root-preserving retry, scalar scheduling, fixed latest/guard health, and one-delete rebuild/replan pass; journal/quarantine/future compatibility gates pending |
...
| TM-SEC-008 | partial | prior state/platform authority, fixed store snapshots, controlled codec/catalog/retention, capacity-one maintenance, and future quarantine | prior evidence plus no main-only backup/caller SQL/path, irreversible poison, fixed slots, full proof revalidation, one-file deletion, exact store-control/reader allowlist, no UI/async/trigger queue, redacted panic containment, linked cancel/non-cancellable publish, and authority mutations; recovery gates pending |
