...
| TM-FUNC-011 | partial | strict Git domain/core/backend/hints plus schema-v13 transactional projection and bounded store capture | Tasks 1-5 domain/aggregate/process/synthetic/privacy/schema/migration/rebuild/append/refresh/invalidation/query contracts pass; public query facade/efficiency join, runtime scheduling/resource evidence, joined status, and UI remain |
...
| TM-PERF-001 | partial | bounded parser/reader/store/engine plus live usage/quota/reminder runtimes, immutable usage/quota/benefit query facade, and short-lived Codex quota transport | prior gates plus quota scale at 32 windows, 1,000 transitions, and 10,000 duplicate polls; benefit scale at 64 lots and 2,048 changes measured 0.842 ms current and 4.904 ms maximum 256-row page; crash-safe reminder 16+48 Windows rounds returned at 117 handles, four threads, USER=1, GDI=0 with a 3,440,640-byte private floor and 5,799,936-byte sampled high; UI/plugin release evidence pending |
...
| TM-DATA-006 | partial | reader/parser/store, engine/runtime, pure pricing engine, and immutable P2 query facade | prior limits plus Git 32+lookahead repositories, 400 inclusive days, eight categories, 16 warnings, explicit daily truncation/range completeness, association ambiguity, hard two-second read cleanup, and owned capture values pass; public Git envelope/efficiency/resource and UI/plugin limits remain |
...
| TM-DATA-010 | partial | non-serializable repository hints, bounded opaque Git values, and durable schema-v13 projection | local-path side-channel privacy plus salted identities, 32-repository/4,096-association/400-day/8-category/16-warning caps, immutable generations, retention truth, exact association clearing/ambiguity, migration rollback, corruption rejection, and bounded read captures pass; runtime population and public join remain |
...
| TM-SEC-002 | partial | current JSONL/store/query boundaries plus strict app-server, quota-benefit, and reminder runtime boundaries validate types, sizes, evidence, identity, continuity, version, deadline, discovery, writer ordering, failure isolation, and public redaction | prior gates plus reminder lease-before-open, store-owned SQL, 256-row maximum, immutable outbox/ack split, sealed non-Debug acknowledgement key, one-batch lease/release/backpressure, stable health codes/counts, panic redaction, joined lifecycle, runtime resource plateau, and four-library release scan pass; future UI/CLI/MCP/plugin boundary suites pending |
...
timer crash-safe durable in-app event delivery with explicit acknowledgement. P3
