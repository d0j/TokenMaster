...
| TM-FUNC-012 | partial | P3-D.0 typed settings, verified SQLite candidates/packages, bounded retention/maintenance, sealed restore/bootstrap, and Task 12A application state owner | Tasks 1-11A plus Task 12A prove pre-open bootstrap, safe-shell isolation, one real backup pipeline/runtime, mandatory verified pre/post-migration points, durable pending-post restart completion, guarded live composition, and clean-after-all-owners-join; typed commands, controlled restart, no-backup reconstruction, health, and UI remain Task 12B+ |
...
| TM-PERF-004 | partial | bounded record/snapshot/package/recovery path plus application-composed capacity-one backup maintenance | prior fixed bounds plus one worker/scheduler, one active/merged follow-up, joined lifecycle, bounded staging/quarantine, path-free streaming verification, worker-only cold catalog verification, one retained catalog projection, atomic deadline-bounded condition wait, and the 36-cycle Windows maintenance resource gate pass; application backup latency and restore/restart-cycle resource evidence remain |
...
| TM-DATA-011 | partial | prior reliable-state layers plus Task 12A application-owned backup/migration composition | prior evidence plus run-state v2 legacy-v1 acceptance and one durable pending migration pair, one complete snapshot-to-retention operation, verified-package catalog binding, safe-shell isolation, mandatory pre/post packages, restart completion, and final clean-after-join; command restart, no-backup reconstruction, UI, and release resource gates pending |
...
| TM-SEC-008 | partial | prior sealed reliable-state authority plus Task 12A application lifecycle and migration gate | prior evidence plus safe mode with zero archive owners, one sealed real backup operation, no cross-layer publication token, durable pre-before-write/post-before-publish migration safety across restart, exact-root atomic waits, partial-owner discard, and clean only after maintenance/controller/runtime join; restore restart, rebuild, UI, and release gates pending |
