...
| TM-FUNC-012 | partial | P3-D.0 library-only `tokenmaster-state` foundation and controlled `tokenmaster-platform` durable files plus future store/app integration | Tasks 1-2 stable error/bound/authority and exact-child staged move/replace contracts pass; settings/package/snapshot/retention/recovery/safe-mode evidence pending |
...
| TM-PERF-004 | planned | P3-D.0 capacity-one maintenance worker and streaming codecs | 10,000-trigger, no-full-buffer, latency, sleep/resume, and repeated memory/handle/thread/USER/GDI gates pending |
...
| TM-DATA-011 | partial | controlled exact-child durable files plus future redundant records, fixed typed packages, self-describing restore points, and recovery journal | bounded staged publication, exact backup, post-commit recovery classification, and 60 Windows kill rounds pass; A/B corruption, package vector, retention, journal crash-phase, quarantine, and compatibility gates pending |
...
| TM-SEC-008 | partial | library-only state authority boundary plus platform-owned exact-child durable staging/replacement and future strict package/quarantine | Task 1's 29-mutation authority gate plus Task 2 link/reparse/type rejection, 32-slot/size/chunk bounds, redaction, exact old-target backup, preserved recovery artifacts, and 60 Windows child-kill rounds pass; package/recovery/adversarial gates pending |
