...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, P3-A-P3-B live shell, P3-C Dashboard, P3-D.1 History, complete P3-D.2 Sessions, P3-D.3 Models, P3-D.4 Projects, and P3-D.5 Activity | prior evidence plus responsive bounded Models/Projects/Activity, explicit evidence/truncation truth, and in-place query-free switching with no extra analytics/Git/Activity query pass; Notifications/Help and P3-E shell integration remain |
...
| TM-UI-001 | partial | P3-A shell, P3-C responsive Dashboard, P3-D.1 History, P3-D.2 Sessions list/detail, P3-D.3 Models, P3-D.4 Projects, and P3-D.5 Activity | prior evidence plus wide/narrow 12-row Recent activity retaining UTC time, model, all five token facts, page/evidence truth, and complete accessible row meaning pass; remaining routes and full accessibility acceptance remain P3-D-P4 |
| TM-UI-002 | partial | immutable runtime/query/product publication plus bounded Dashboard/History/Sessions/Models/Projects/Activity route-only Slint application | epoch/newer-generation replacement includes one capped History, Sessions list/detail, Models, Projects, and Activity model; route-only updates perform no query/model rebuild/window recreation/timer/animation; hot skin/locale/layout settings remain P4 |
| TM-PERF-001 | partial | bounded backend, one-current product/Desktop projection, bounded query/operation workers, one snapshot/runtime-observation/event gate, and capped Dashboard/History/Sessions/Models/Projects/Activity models | prior gates plus 12 Activity rows, one existing first-page request, replace-only models, zero route-time work/polling, and 10,000-replacement release pass; measured visible-paint and long-run release evidence remain P4/P6 |
...
| TM-DATA-001 | partial | prior privacy boundaries plus path-private app root and bounded Dashboard/History/Sessions/Models/Projects/Activity models | Activity adds only UTC timestamp, canonical model, token aggregates, evidence, and page truth; no scope/event/dataset/provider/profile/source/account/session/project identity, key/cursor/fingerprint, path, content, or authority crosses Slint; remaining P3-D-P5 surfaces repeat gates |
...
| TM-DATA-006 | partial | prior bounded publication plus current Dashboard/History/Sessions/Models/Projects/Activity projections | prior caps plus 12 newest-first Activity rows over the existing page, explicit `has_more`/frontend truncation, aggregate-rebuild availability, and no prior page/selection/filter/cache pass; later pagination, rhythm aggregates, and P4 switch limits remain |
...
| TM-SEC-002 | partial | prior strict boundaries plus separate app/frontend authority and privacy-bounded Dashboard/History/Sessions/Models/Projects/Activity/reliable-state UI | Activity has one mapping/application site, no extra query or raw-event export, and zero scope/event/dataset/provider/profile/source/session/project/path/key/cursor/fingerprint/content authority; remaining P3-D-P5/plugin and interactive accessibility suites remain |
