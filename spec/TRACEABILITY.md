...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, P3-E.1 route command palette, app-owned in-app expiry presentation, and global reminder settings editor | the route-only palette reuses the 11-route projection with a 64-scalar query and one replace-only model; global portable desired state synchronizes reminders through startup, Save, and confirmed import; per-scope editing, OS/tray, usage alerts, and remaining P3-E remain |
| TM-FUNC-005 | partial | `crates/probe-app` evidence plus separate `crates/desktop` frontend and sole `crates/app` production binary | software-only app composition and the same-window P3-E.1 palette pass; M0 evidence remains separate and production tray/hotkey/single-instance/startup/compact lifecycle remain P3-E/P6 |
...
| TM-UI-001 | partial | complete bounded data routes, static Help/About, P3-E.1 command palette, and transient accessible expiry panel | real headless Slint proves focused text plus keyboard/pointer/accessibility route activation and all notification rows applied before visibility receipt; unified localization and full interactive accessibility acceptance remain P4/P6 |
| TM-UI-002 | partial | immutable publication, bounded routes/palette, and independent notification epoch bridge | the final overlay keeps one 64-scalar query and one 11-row replace-only model refreshed from accepted snapshots; notification application/receipt remains separate; hot skin/locale/layout settings remain P4 |
| TM-PERF-001 | partial | bounded backend, one-current projections, capped data models including P3-E.1, one notification receipt worker, and constant-capacity bridges | a 10,000-scalar palette input retains 64 scalars and at most 11 rows; 10,000 notification pumps retain one lease/presentation and one transient 256-row model; long-run P4/P6 resource evidence remains |
...
| TM-DATA-001 | partial | prior privacy boundaries plus path-private app root, bounded data models, static Help/About, and route-only P3-E.1 palette | the palette contains only fixed route keys/labels/state/reasons plus a transient 64-scalar user filter and exposes no general command, content, credential, path, or identity surface; remaining P3-E-P5 surfaces repeat gates |
