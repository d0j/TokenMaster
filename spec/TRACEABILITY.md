...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, P3-E.1 route command palette, P3-E.2 compact quota mode, P3-E.3 typed tray lifecycle, P3-E.4 current-session activation, app-owned in-app expiry presentation, and global reminder settings editor | palette and compact reuse existing route/quota truth; one five-intent tray router and one capacity-one secondary/hotkey Show bridge reach only the same window and joined shutdown; global portable desired state synchronizes reminders through startup, Save, and confirmed import; per-scope editing, reminder OS/tray delivery, usage alerts, and remaining P3-E remain |
| TM-FUNC-005 | partial | `crates/probe-app` evidence plus separate `crates/desktop` frontend, sole `crates/app` production binary, and `tokenmaster-platform` current-session owner | P3-E.1-P3-E.4 software composition proves same-window palette/compact/tray plus early current-session arbitration, fixed hotkey, one pending activation/task, joined teardown, 10,000-signal coalescing, and 256 test-owner resource cycles; startup and live multi-process/focus/hotkey/ACL/sleep/resource acceptance remain P3-E/P6 |
...
| TM-UI-001 | partial | complete bounded data routes, static Help/About, P3-E.1 command palette, P3-E.2 compact quota mode, P3-E.3 tray lifecycle, P3-E.4 Show/restore/focus activation, and transient accessible expiry panel | real Slint proves palette/compact activation, explicit unknown ratio, exact size restore, exact five-action native menu mapping, and bounded hotkey/secondary delivery to the existing window; unified localization and live Windows focus/accessibility acceptance remain P4/P6 |
...
| TM-PERF-001 | partial | bounded backend, one-current projections, capped P3-E.1/P3-E.2 models, queue-free P3-E.3 lifecycle router, one P3-E.4 native thread, one notification receipt worker, and constant-capacity bridges | palette/compact/notification stress bounds remain; current-session activation adds one auto-reset bit, one pending bit, one scheduled task, no payload/queue/timer/polling, 10,000-signal coalescing, panic containment, and 256 test-owner resource return; live hotkey/multi-process and long-run P4/P6 evidence remain |
...
| TM-SEC-002 | partial | strict app/Desktop authority split, privacy-bounded data UI, Help/About, P3-E palette/compact surfaces, and present-only notification DTO | only app touches reminder mutation/take/ack/release; Desktop palette and compact modes add no query/native/mutation authority, and the visible-Pending acknowledgement remains bounded without runtime/store/SQL/path/private-ID authority; remaining P3-E-P5/plugin and interactive accessibility suites remain |
