...
| TM-DATA-004 | implemented | atomic current append, scoped complete-only scan finalization, scan-bound and compatibility staging, exact preparation, paged seal, retained projection promotion, and staging discard | scan lifecycle/begin-fault/membership/reopen/zero-source/300-source/Codex pipeline contracts pass; pruning remains P1-B.3 |
...
| TM-SEC-004 | partial | transactional scoped scan and replay authority, CAS/preparation, immutable legacy, exact paged seal, explicit carry-forward, atomic rollback, and exact-epoch discard | P1-B.1/B.2 prove complete-only presence, exact binding, stale-membership rejection, zero-source retention, and fault rollback; pruning/live runtime remain |
...
Bounded scan-history pruning and the production runtime engine remain P1-B.3 and later.
