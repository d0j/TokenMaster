...
| TM-FUNC-012 | partial | prior reliable-state stack plus Task 12A, Task 12B.1 command/restart, Task 12B.2a selected restore, Task 12B.2b.1 worker/config core, and Task 14 sealed file selection | prior evidence plus one joined capacity-one operation worker, real off-UI manual backup binding, 2 MiB typed config core, current-directory restore pinning, Windows Common Item Dialog, no-follow bounded input, parent/target-bound staged output, displaced-target rollback, and deterministic selector/race contracts pass; application/UI config binding, verify/restore/rebuild commands, no-backup reconstruction, health, and UI remain |
...
| TM-PERF-004 | partial | bounded reliable-state path plus backup maintenance, joined constant-state app operation worker, and immutable catalog snapshots | prior fixed bounds plus one operation worker/thread, capacity-one wake/latest completion, one command active/follow-up under 10,000 hints, off-UI manual backup, 2 MiB config ceiling, bounded staging/quarantine, short-held catalog mutex, atomic waits, and the 36-cycle Windows maintenance resource gate pass; native-dialog and selected-restore operation resource evidence remain |
...
| TM-DATA-011 | partial | prior reliable-state layers plus Task 12A, Task 12B.1, Task 12B.2a selected restore, and Task 14 selection capabilities | prior evidence plus opaque package identity across catalog reorder, admitted-before-pin retention protection, journaled selected replacement, accepted recovery generation, repeated legacy migration safety, and transient path-free no-follow input/parent-and-target-bound output with displaced recovery evidence; worker/UI bindings, no-backup reconstruction, and release resource gates pending |
...
| TM-SEC-008 | partial | prior sealed state plus Task 12A, Task 12B.1, Task 12B.2a selected restore, and Task 14 platform file capabilities | prior evidence plus path/digest-private restore identity binding, deletion-serialized late pin, protected safety point, receipt-before-restart ordering, restored-legacy migration replay, old-owner join/fresh guard, exact typed native filters, no-follow single-link bounded input, Windows handle-pinned stage cleanup, post-syscall displaced-identity rollback, and source-policy contracts; Unix cleanup hardening, rebuild, application/UI bindings, reconstruction, and release gates pending |
