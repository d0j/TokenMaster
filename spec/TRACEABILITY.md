...
| TM-FUNC-010 | partial | strict provider-neutral benefit lots, Codex reset-credit normalization, schema-v11 inventory/history/profile/due storage, and immutable current/history query facade; reminder runtime/UI and activation later | domain/core/Codex/store/query privacy, FEFO, freshness/completeness, profile, nearest expiry/due, revision-bound paging, corruption, restart, scale, and resource fixtures pass; one-timer delivery remains next |
...
| TM-DATA-001 | partial | domain/provider/Codex/store privacy boundaries, pseudonymous quota account normalization, and count-only quota-runtime health | adversarial/debug/path/privacy tests prove no executable/archive path, Codex home, email, account/window identity, quota value, raw frame, credit ID, or inner provider/store error escapes connector/runtime surfaces; future UI/automation surfaces must repeat gates |
...
| TM-DATA-009 | partial | typed provider benefit inventory, pure reconciliation/reminder planning, built-in Codex normalization, strict schema-v11 current/history/profile/due/delivery foundation, and immutable query facade; activation later | prior store evidence plus FEFO current snapshots, explicit absent/stale/partial/unknown facts, inherited/override profile metadata, nearest expiry/due, scope/revision-bound 256-row history, restart/concurrent-snapshot/deadline cleanup, 64-lot/2,048-change scale, corruption rejection, generation neutrality, no-usage-scan, and resource return pass; delivery runtime, UI, intents/receipts remain |
...
runtime health are implemented. Separate banked-reset inventory/reminders and P3 UI
evidence remain.
M0 interactive/
