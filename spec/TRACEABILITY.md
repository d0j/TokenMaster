...
| TM-FUNC-007 | partial | accounting lineage/classifier plus bounded P0-D replay archive lifecycle implemented | lifecycle contracts pass, but P0-D.1 must remove the 256-file manifest blocker before P0-E cross-crate proof |
...
| TM-DATA-004 | partial | atomic current append plus fixed-manifest staging, exact seal, rollback-safe promotion, and staging discard implemented | lifecycle/rollback contracts pass, but the current 256-file manifest blocks large valid histories; P0-D.1 schema v3/all-source/page gate is next |
...
| TM-SEC-004 | partial | transactional CAS, immutable legacy, exact seal, atomic promotion, injected rollback, and exact-epoch staging discard implemented | archive mutation/recovery contracts pass; P0-E proves failed/cancelled cross-crate behavior and P1 owns runtime policy |
...
are implemented. P0-E preflight found the 256-file manifest product blocker; P0-D.1
schema v3, disk-backed all-source begin, and paged seal must pass before the P0-E
transactional Codex-to-archive proof. The production runtime engine remains P1.
