...
| TM-FUNC-003 | partial | domain/accounting/Codex parser, schema-v9 price facts, indexed aggregate query facade, P3-C Dashboard, P3-D.1 History, P3-D.2 Sessions list/detail, and P3-D.3 Models | prior evidence plus one shared recent-30-day Model/Project capture and a visible 64-row complete model token/cost breakdown pass; interactive ranges, remaining exploration, and CLI/MCP remain |
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, P3-A-P3-B live shell, P3-C Dashboard, P3-D.1 History, complete P3-D.2 Sessions, and P3-D.3 Models | prior evidence plus responsive bounded Models, explicit range/evidence/truncation truth, and in-place query-free route switching with no third analytics query pass; Projects/Activity/Notifications/Help and P3-E shell integration remain |
...
| TM-UI-001 | partial | P3-A shell, P3-C responsive Dashboard, P3-D.1 responsive History, P3-D.2 responsive Sessions list/detail, and P3-D.3 responsive Models | prior evidence plus a wide/narrow 64-row Models route retaining input/cached/output/reasoning/total/cost/events, range/evidence, visible and accessible partial plus calculated/reported/mixed cost evidence, and truncation pass; remaining routes and full accessibility acceptance remain P3-D-P4 |
| TM-UI-002 | partial | immutable runtime/query/product publication plus bounded Dashboard/History/Sessions/Models route-only Slint application | epoch/newer-generation replacement includes one capped History, Sessions list/detail, and Models model; route-only updates perform no query/model rebuild/window recreation/timer/animation, and Models/Projects share the current recent envelope; hot skin/locale/layout settings remain P4 |
| TM-PERF-001 | partial | bounded backend, one-current product/Desktop projection, bounded query/operation workers, one snapshot/runtime-observation/event gate, and capped Dashboard/History/Sessions/Models models | prior burst gates plus 30 History rows, 64 Sessions rows, 32+32 detail rows, 64 Models rows, exactly two analytics queries, replace-only models, and route-only hot paths pass; measured visible-paint and long-run release evidence remain P4/P6 |
...
| TM-PERF-003 | partial | immutable engine/product publication, indexed facades, epoch/stale-safe desktop projection, bounded query controller, newest-only bridge, and live app hints | prior evidence plus one shared exact recent-days History/Models/Projects envelope, no third analytics query, sequential Sessions page, and latest-only exact detail reads through the existing worker pass; P4 paint and final release evidence remain |
...
| TM-DATA-001 | partial | prior privacy boundaries plus path-private app root and bounded Dashboard/History/Sessions/Models models | Models adds only canonical bounded model keys and aggregate range/evidence/event/token/cost facts; no provider/profile/source/account/workspace/project/session identity, opaque key/cursor, path, content, or authority crosses Slint; remaining P3-D-P5 surfaces must repeat gates |
...
| TM-DATA-006 | partial | prior bounded publication plus current Dashboard/History/Sessions/Models projections | prior caps plus one 64-row Models projection over at most 256 backend Model and 256 prefetched Project items, explicit two-level truncation, and no prior dataset/filter/sort cache pass; Projects, Activity, later pagination, and P4 switch limits remain |
...
| TM-SEC-002 | partial | prior strict boundaries plus separate app/frontend authority and privacy-bounded Dashboard/History/Sessions/Models/reliable-state UI | prior exact-detail authority gates plus Models canonical-key-only rows, one mapping/application site, no third query, and zero provider/profile/private-ID/path/key/cursor/SQL/content authority pass; remaining P3-D-P5/plugin and interactive accessibility suites remain |
