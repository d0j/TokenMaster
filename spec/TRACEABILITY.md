...
| TM-FUNC-009 | partial | exact quota domain/evaluator, strict schema-v10 storage, bounded retention, defensive reads, and immutable public query facade; provider transport/UI planned | prior quota gates plus request-ordered explicit missing results, provider-defined freshness, worst truthful quality, opaque revision/filter continuation, generation neutrality, adversarial inference matrix, 32-window/1,000-transition/10,000-poll release scale, Windows resource plateau, and offline authority audit pass; permitted Codex transport and UI pending |
...
| TM-PERF-001 | partial | bounded parser/reader/store/engine plus live runtime and query facade | prior gates plus quota scale at 32 windows, 1,000 transitions, and 10,000 duplicate polls; maximum visible write 3.429 ms, duplicate 0.228 ms, current-32 2.774 ms, history-256 1.256 ms; repeated current/history/reopen cycles preserve private-memory, handle, thread, USER, and GDI plateau; UI/plugin release evidence pending |
...
| TM-SEC-001 | partial | local-only product, no listener, deterministic offline pricing, and offline quota core | pricing and quota dependency/source/release-library audits prove no runtime pricing or quota-core network/browser/shell path; permitted credential-free local/official quota source and future MCP stdio network-denial tests pending |
| TM-SEC-002 | partial | current JSONL/store boundaries plus complete quota core/query boundary validate types, sizes, evidence, identity, state continuity, and public redaction | prior quota gates plus fixed query-only SQL, revision/filter-bound cursors, generation neutrality, public Debug privacy, adversarial no-inference matrix, resource/scale bounds, and offline authority audit pass; future provider/config/CLI/MCP/plugin boundary suites pending |
...
resource evidence, and offline authority audit are implemented. Permitted Codex quota
transport and separate banked-reset inventory/reminders remain. P3 UI
