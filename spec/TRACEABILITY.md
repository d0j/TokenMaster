...
| TM-FUNC-003 | partial | domain/accounting/Codex parser, schema-v9 price facts, indexed aggregate query facade, P3-C Dashboard, P3-D.1 History, and P3-D.2a Sessions list | exact token/cost overview, daily-series, breakdown, session page/detail query mapping plus visible today, exact recent-30-day History, and newest 64-row Sessions page with full token/cost facts and `has_more` pass; exact interactive detail, ranges, remaining exploration, and CLI/MCP remain |
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, P3-A-P3-B live shell, P3-C Dashboard, P3-D.1 History, and P3-D.2a Sessions route | prior Dashboard/History evidence plus independent bounded Sessions section, responsive real list, explicit page completeness, and in-place query-free route switching pass; exact detail, Models/Projects/Activity/Notifications/Help and P3-E shell integration remain |
...
| TM-UI-001 | partial | P3-A shell, P3-C responsive Dashboard, P3-D.1 responsive History, and P3-D.2a responsive Sessions list | prior evidence plus exact range/timezone/evidence, 30-day History trend/table, and wide/narrow 64-row Sessions token/cost table with explicit `has_more`, unavailable truth, and accessible row meaning pass; exact detail, remaining routes, and full accessibility acceptance remain P3-D-P4 |
| TM-UI-002 | partial | immutable runtime/query/product publication plus bounded Dashboard/History/Sessions route-only Slint application | accepted-generation replacement includes one capped History and one capped Sessions model; route-only updates perform no query/model rebuild/window recreation/timer/animation; generation-bound detail and hot skin/locale/layout settings remain P3-D.2b/P4 |
| TM-PERF-001 | partial | bounded backend, one-current product/Desktop projection, bounded query/operation workers, one snapshot/runtime-observation/event gate, and capped Dashboard/History/Sessions lists | prior burst gates plus fixed 30-row History and 64-row Sessions requests/projections/models, sequential existing-worker queries, and route-only hot paths pass; measured visible-paint and long-run release evidence remain P4/P6 |
...
| TM-DATA-001 | partial | prior privacy boundaries plus path-private app root and identity-free Dashboard/History/Sessions models | History adds only civil dates and aggregate values; Sessions adds only aggregate timestamps/counts/tokens/cost and `has_more`; projection/UI contracts retain no raw session/event/account/workspace/project/source identity, key, or cursor; remaining P3-D-P5 surfaces must repeat gates |
...
| TM-DATA-006 | partial | prior bounded publication plus current Dashboard and independent History/Sessions projections/models | prior exact Dashboard caps plus one 30-row newest-first History model and one 64-row newest-first Sessions model with explicit continuation and no prior-page/detail cache pass; exact session detail, later pagination, other P3-D views, and P4 switch limits remain |
...
| TM-SEC-002 | partial | prior strict boundaries plus separate app/frontend authority and identity-free Dashboard/History/Sessions/reliable-state UI | History and Sessions add no path/ID/key/cursor/SQL/content authority and use the same worker/snapshot/event surfaces; exact detail must resolve ordinal/generation inside the controller; remaining P3-D-P5/plugin and interactive accessibility suites remain |
