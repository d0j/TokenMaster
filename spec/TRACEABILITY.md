...
| TM-FUNC-003 | partial | domain/accounting/Codex parser, schema-v9 price facts, indexed aggregate query facade, P3-C Dashboard, P3-D.1 History, P3-D.2 Sessions list/detail/later-page navigation, P3-D.3 Models, P3-D.4 Projects, and P3-D.5 Recent activity | P3-D interactive History ranges now publish fixed 1/7/30 rolling civil-day envelopes (default/max 30) shared identically by History/Models/Projects with exact epoch/product/range fences, accepted-only publication, bounded replacement, and terminal rollback; rhythm aggregation, remaining exploration, and CLI/MCP remain |
...
| TM-DATA-006 | partial | prior bounded publication plus current data projections, replace-only P3-D.2c Sessions navigation, interactive History range envelope, static Help/About, and P3-E.2 compact projection | prior caps remain; History range selection replaces one shared at-most-30-day envelope through one worker/slot, with accepted-only publication, exact epoch/product/range fences, and terminal rollback; Sessions retains one at-most-64 page and no cursor history, while Compact reuses the current at-most-32 quota rows; rhythm aggregates and P4 switch limits remain |
