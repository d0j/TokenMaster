...
| TM-FUNC-003 | partial | domain/accounting/Codex parser, schema-v9 price facts, indexed aggregate query facade, P3-C Dashboard, P3-D.1 History, P3-D.2 Sessions list/detail, P3-D.3 Models, and P3-D.4 Projects | prior evidence plus one shared recent-30-day Model/Project capture and visible bounded model/project token-cost breakdowns pass; interactive ranges, remaining exploration, and CLI/MCP remain |
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, P3-A-P3-B live shell, P3-C Dashboard, P3-D.1 History, complete P3-D.2 Sessions, P3-D.3 Models, and P3-D.4 Projects | prior evidence plus responsive bounded Models/Projects, explicit dual-range/evidence/truncation truth, and in-place query-free switching with no extra analytics/Git query pass; Activity/Notifications/Help and P3-E shell integration remain |
...
| TM-UI-001 | partial | P3-A shell, P3-C responsive Dashboard, P3-D.1 History, P3-D.2 Sessions list/detail, P3-D.3 Models, and P3-D.4 Projects | prior evidence plus wide/narrow 32-row Projects retaining full usage mix and separately labelled commits/lines/efficiency with complete accessible row meaning pass; remaining routes and full accessibility acceptance remain P3-D-P4 |
| TM-UI-002 | partial | immutable runtime/query/product publication plus bounded Dashboard/History/Sessions/Models/Projects route-only Slint application | epoch/newer-generation replacement includes one capped History, Sessions list/detail, Models, and Projects model; route-only updates perform no query/model rebuild/window recreation/timer/animation; hot skin/locale/layout settings remain P4 |
| TM-PERF-001 | partial | bounded backend, one-current product/Desktop projection, bounded query/operation workers, one snapshot/runtime-observation/event gate, and capped Dashboard/History/Sessions/Models/Projects models | prior gates plus 32 Projects rows, at most 1,024 exact alias comparisons, exactly two analytics and one existing Git query, replace-only models, and route-only hot paths pass; measured visible-paint and long-run release evidence remain P4/P6 |
...
| TM-DATA-001 | partial | prior privacy boundaries plus path-private app root and bounded Dashboard/History/Sessions/Models/Projects models | Projects adds only safe aliases/unassociated state and aggregate dual-window facts; no repository/association/dataset/private ID, provider/profile/source/account/session identity, key/cursor, path, content, or authority crosses Slint; remaining P3-D-P5 surfaces repeat gates |
...
| TM-DATA-006 | partial | prior bounded publication plus current Dashboard/History/Sessions/Models/Projects projections | prior caps plus 64 Models and 32 Projects rows over one 256+lookahead recent envelope and at most 32 Git repositories, explicit usage/Git truncation, and no prior dataset/filter/sort cache pass; Activity, later pagination, and P4 switch limits remain |
...
| TM-SEC-002 | partial | prior strict boundaries plus separate app/frontend authority and privacy-bounded Dashboard/History/Sessions/Models/Projects/reliable-state UI | exact-alias-only Projects join, one mapping/application site, no extra query, separate ranges, and zero repository/association/dataset/private-ID/path/key/cursor/SQL/content authority pass; remaining P3-D-P5/plugin and interactive accessibility suites remain |
