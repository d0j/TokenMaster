...
| TM-FUNC-005 | partial | `crates/probe-app` evidence plus separate `crates/desktop` frontend, sole `crates/app` production binary, and `tokenmaster-platform` current-session/current-user startup owners | P4-B durable density proves one three-value v2 field, in-memory v1 migration, source-version-bound typed packages, admission-before-apply, one style owner, one-active/one-pending latest payload, and 10,000 switches. Skins/layouts/schemes/locales, typography/row sizing, accessibility/DPI/paint/resource verification, and live Windows acceptance remain P3-E/P4/P6 |
...
| TM-UI-002 | partial | immutable publication, bounded routes/palette/compact projection, independent notification epoch bridge, and P4-B durable density owner | P4-B fixes Comfortable/Compact/Ultra compact keys and indices, v2 persistence/migration, saved/saving/not-saved semantics, seven token tables, admission-before-apply, one owner, and a 10,000 latest-payload contract without new authority. Full skins/layouts/color schemes/locales, typography/row-size behavior, accessibility/DPI/paint/resource verification, and hot skin/locale/layout settings remain P4 |
