...
| TM-FUNC-012 | partial | P3-D.0 typed settings, store-owned verified SQLite candidates, fixed typed packages, and manual age protection over crate-private records and controlled durable files | Tasks 1-7 pass stable authority/settings, WAL-consistent Online Backup, independent verification/compaction/identity, deterministic `.tmconfig`/`.tmbackup`, verified-source-bound age v1 export/import, poisoned failure output, and privacy/adversarial gates; retention/recovery/safe-mode evidence pending |
...
| TM-DATA-011 | partial | controlled durable files/readers, redundant records, typed settings, verified SQLite candidates, and fixed v1 typed packages plus future catalog/journal | prior evidence plus exact `TMPKG001`/`TMMNF001`/typed-entry/`TMEND001` vectors, frozen golden length/SHA-256, all profiles/purposes, strict single-frame Zstd, descriptor/entry/package binding, and poisoned incomplete output pass; retention, journal transitions, quarantine, and future compatibility gates pending |
...
| TM-SEC-008 | partial | prior state/platform authority, fixed store snapshots, controlled streaming package codec, and future quarantine | prior evidence plus no main-only backup/caller SQL/name/path, defensive SQLite verification, pinned minimal-feature Zstd, exact bounds/window/content size/checksum/end/digest gates, real bomb/missing-end mutations, irreversible stage poison, private wire/errors, 36 authority mutations, and final Task 6 review `Ready: Yes`; recovery gates pending |
