...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, and app-owned in-app expiry presentation | visible application precedes durable acknowledgement; failed-presentation release/re-pump, terminal-ack release without re-presentation, panic rollback, 60-second transient acknowledgement retry, one worker, one transient model, and 256-row caps pass; settings editing, OS/tray, usage alerts, and P3-E remain |
...
| TM-FUNC-010 | partial | prior benefit/reminder stack, expiry-safety center, and app-owned in-app presentation receipts | separate lots/expiry/profile truth plus bounded visible presentation, post-apply ack, panic-safe failed-presentation release/re-pump, terminal-ack release without re-presentation, and exact transient retry pass; settings synchronization/editing, snooze, quiet hours, OS delivery, and activation remain |
...
| TM-UI-001 | partial | complete bounded data routes, static Help/About, and transient accessible expiry panel | real headless Slint proves all rows applied before visibility receipt and dismissal clears the model; unified localization and full interactive accessibility acceptance remain P4/P6 |
| TM-UI-002 | partial | immutable publication plus bounded routes and independent notification epoch bridge | one model/count/visible application, one in-flight presentation, one-shot receipt, no UI timer/polling/auto-hide, and route-only navigation pass; hot skin/locale/layout settings remain P4 |
| TM-PERF-001 | partial | bounded backend, one-current projections, capped data models, one notification receipt worker, and constant-capacity bridges | 10,000 notification pumps retain one lease/presentation, one transient 256-row model, and no retained batch in the worker; long-run P4/P6 resource evidence remains |
...
| TM-DATA-009 | partial | typed benefit inventory, schema-v12 due/outbox/ack, durable reminder runtime, and app-owned visible presentation | outbox-before-event, restart replay, capacity-one lease, visible-before-ack, retry/release/shutdown and identity-free mapping pass; settings, OS delivery, and activation intents/receipts remain |
...
| TM-SEC-002 | partial | strict app/Desktop authority split, privacy-bounded data UI, Help/About, and present-only notification DTO | only app touches reminder take/ack/release; Desktop has zero runtime/store/SQL/path/private-ID authority and one checked weak-window epoch; P3-E-P5/plugin and interactive accessibility suites remain |
