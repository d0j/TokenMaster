...
| TM-FUNC-007 | partial | accounting lineage/classifier plus v2 replay archive and staged append implemented | replay archive contracts cover migration, version ownership, staging invisibility, classification, rollback, and selection; late relation continuation, seal/promotion, and P0-E remain |
...
| TM-DATA-007 | partial | replay facts/classifier persisted in a strict v2 private overlay with deterministic eligible selection | migration/read/staged-append/classification tests pass; durable late-ancestry continuation, seal/promotion, and P0-E remain |
...
| TM-SEC-004 | partial | current and replay append transactions/CAS, immutable legacy snapshot, fixed manifest, version fail-closed checks | replay rollback and invisibility tests pass; seal/promotion failure injection and failed-scan reconciliation remain |
...
P0-D schema, migration, archive state, fixed-manifest staging, and classified append
are implemented; late relation continuation and atomic seal/promotion are next.
