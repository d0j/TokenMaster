...
| TM-FUNC-009 | partial | exact quota domain/evaluator, strict schema-v10 storage, bounded retention, defensive reads, immutable public query facade, and built-in official Codex quota normalizer/transport; runtime publication/UI planned | prior quota gates plus strict app-server `0.144.1` handshake/schema/version/size/deadline/process cleanup, live two-window smoke, adversarial envelopes, repeated success/error/timeout Windows resource plateau, and release authority audit pass; executable discovery, refresh scheduling/store publication, and UI remain |
...
| TM-PERF-001 | partial | bounded parser/reader/store/engine plus live runtime, query facade, and short-lived Codex quota transport | prior gates plus quota scale at 32 windows, 1,000 transitions, and 10,000 duplicate polls; maximum visible write 3.429 ms, duplicate 0.228 ms, current-32 2.774 ms, history-256 1.256 ms; repeated live Codex polls completed in 0.70-0.94 seconds and repeated success/error/timeout transport cycles preserve parent private-memory/handle/thread/USER/GDI plateau; UI/plugin release evidence pending |
...
| TM-DATA-001 | partial | domain/provider/Codex/store privacy boundaries plus pseudonymous quota account normalization | adversarial/debug/path/privacy tests prove no executable path, Codex home, email, raw frame, credit ID, or inner provider error escapes; future runtime/UI/automation surfaces must repeat gates |
...
| TM-DATA-005 | implemented | writable usage/quota store plus separate `UsageReadStore` | strict schema v10 retaining dataset/token/price contracts plus quota-owned objects, exact v8 and v9 rollback migrations, read-only query-only defensive 4-MiB/no-migration/no-checkpoint policy, and fixed quota-only current/history captures pass |
...
| TM-SEC-001 | partial | local-only product, no listener, deterministic offline pricing/quota core, and credential-blind official Codex child transport | pricing/quota-core audits plus Codex transport dependency/source/release-library audit prove no TokenMaster HTTP/browser/cookie/private-endpoint/shell/socket path; future MCP stdio network-denial tests pending |
| TM-SEC-002 | partial | current JSONL/store/query boundaries plus strict app-server quota wire/process boundary validate types, sizes, evidence, identity, continuity, version, deadline, and public redaction | prior quota gates plus fixed protocol IDs/methods, unknown-field rejection, 256-KiB/1-MiB/64-frame caps, 32 normalized windows, 64 transient credits, timeout/reap/join, live smoke, adversarial envelopes, and resource plateau pass; future discovery/config/CLI/MCP/plugin boundary suites pending |
...
and live-verified for the pinned version. Executable discovery, dedicated refresh
scheduling/writer coordination, separate banked-reset inventory/reminders, and P3 UI
evidence remain.
M0 interactive/
