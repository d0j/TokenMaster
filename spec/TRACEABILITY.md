...
| TM-REL-001 | partial | M0 scripts, deterministic product producer/validator, product receipt schemas, separate P3-D.0 developer receipt, and `P3E_ACCEPTANCE.md` | the P6 unsigned producer binds one clean commit, canonical MSVC executable, closed content manifest, and package receipt; authenticated external receipts, current schema-4 P3-D.0 exact-commit receipt, signing, and final product acceptance remain pending |
