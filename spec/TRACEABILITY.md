...
| TM-FUNC-002 | implemented | Codex reader plus store/runtime incremental path | zero-payload unchanged, exact tail bytes, multi-batch restart, new/missing source, replacement/truncation/profile-scope and malformed-input recovery contracts; strict-superset staging extends in place, replacement publication remains private until atomic promotion, and cancellation is observed between bounded replay transactions |
...
| TM-PERF-001 | partial | bounded backend, one-current projections, capped P3-E.1/P3-E.2 models, queue-free P3-E.3 lifecycle router, one P3-E.4 native thread, one notification receipt worker, constant-capacity bridges, bounded staging replay, retained query-resource warm-up, and first-safe-import status | the parser and synthetic cold contracts remain fast; a real 4,007-source run exposes safe Partial data during the active worker at about 27.7 MiB peak private memory, but completes only 236 sources in 60 seconds. JSON prefilter, soft-batch, index, verification-read, and cache tuning were rejected; transactional bounded bulk ingest and complete cold-import evidence remain open |
