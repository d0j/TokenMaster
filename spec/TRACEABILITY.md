...
| TM-FUNC-010 | partial | strict provider-neutral benefit lots, Codex reset-credit normalization, schema-v11 inventory/history/profile/due storage, immutable current/history query facade, and Codex runtime publication; reminder runtime/UI and activation later | domain/core/Codex/store/query privacy plus one-poll/one-lease/one-open quota-benefit publication, separate transaction/health/failure/last-success, restart, contention, cancellation, retry, scale, and resource fixtures pass; one-timer delivery remains next |
...
| TM-PERF-001 | partial | bounded parser/reader/store/engine plus live usage/quota runtimes, immutable usage/quota/benefit query facade, and short-lived Codex quota transport | prior gates plus quota scale at 32 windows, 1,000 transitions, and 10,000 duplicate polls; benefit scale at 64 lots and 2,048 changes measured 0.842 ms current and 4.904 ms maximum 256-row page; combined quota-benefit runtime 16+48 Windows rounds returned at 131 handles, four threads, USER=1, GDI=0 with a 3,432,448-byte private floor and 6,139,904-byte sampled high; UI/plugin release evidence pending |
...
| TM-DATA-001 | partial | domain/provider/Codex/store privacy boundaries, pseudonymous quota account normalization, and count-only quota/benefit runtime health | adversarial/debug/path/privacy tests prove no executable/archive path, Codex home, email, account/window/lot identity, quota/benefit value, raw frame, credit ID, or inner provider/store error escapes connector/runtime surfaces; future UI/automation surfaces must repeat gates |
...
| TM-DATA-005 | implemented | writable usage/quota/benefit store plus separate `UsageReadStore` | strict schema v11 retaining dataset/token/price/quota contracts plus benefit-owned objects, exact v8/v9/v10 rollback migrations, read-only query-only defensive 4-MiB/no-migration/no-checkpoint policy, and fixed quota/benefit current/history captures pass |
| TM-DATA-006 | partial | reader/parser/store, engine/runtime, pure pricing engine, and immutable P2 query facade | prior limits plus 401-target/512-key range cost batches, 256-target breakdown/session batches, exact omitted counters, calendar composition, optional 400-point daily series, four breakdowns, scope-bound cursors, 32 scopes/windows, quota and benefit 256+1 history pages, 64 benefit lots, 16 warnings, two-second reads, owned values, and public quota/benefit envelopes pass; UI/plugin limits pending |
...
| TM-DATA-009 | partial | typed provider benefit inventory, pure reconciliation/reminder planning, built-in Codex normalization, strict schema-v11 current/history/profile/due/delivery foundation, immutable query facade, and quota-runtime publication; activation later | prior store/query evidence plus one poll/lease/open, independent quota/benefit transactions and revisions, per-domain health/failure/last-success, quota-prefix plus benefit success, quota success plus benefit failure, duplicate restart, cancellation, contention/retry, and combined resource return pass; delivery runtime, UI, intents/receipts remain |
| TM-SEC-001 | partial | local-only product, no listener, deterministic offline pricing/quota core, credential-blind official Codex child transport, and shell-free quota runtime | pricing/quota-core/transport audits plus quota-runtime dependency/source/release-library audit prove no TokenMaster HTTP/browser/cookie/private-endpoint/credential-file/shell/socket/direct-SQL path; future MCP stdio network-denial tests pending |
| TM-SEC-002 | partial | current JSONL/store/query boundaries plus strict app-server and quota-benefit runtime boundaries validate types, sizes, evidence, identity, continuity, version, deadline, discovery, writer ordering, failure isolation, and public redaction | prior quota gates plus fixed protocol IDs/methods, unknown-field rejection, 256-KiB/1-MiB/64-frame caps, 32 normalized windows, 64 transient credits, exact native discovery, no explicit fallback, I/O-before-lease, separate exact transactions, count-arithmetic validation, cancellation-before-publication, timeout/reap/join, adversarial envelopes, and connector/runtime resource plateaus pass; future CLI/MCP/plugin boundary suites pending |
| TM-SEC-003 | implemented | provider/Codex/store/engine errors, value types, redacted worker panic boundary, and redacted quota identities/captures/transport | serialized/debug privacy, path-redaction, sealed identity, path-substitution, raw-archive-write compile-fail, redacted observation/scope/epoch/transition/cursor/filter/command output, stable quota/transport error codes, fixed panic/fault/process cleanup, and panic-strategy compile guard contracts |
...
runtime health are implemented. Benefit inventory Tasks 1-6 now cover values, pure
reconciliation/reminder planning, Codex normalization, schema-v11 persistence/
retention, immutable query snapshots, and combined quota-runtime publication.
One-timer reminder delivery and P3 UI evidence remain.
M0 interactive/
