...
| TM-FUNC-009 | partial | exact quota domain/evaluator plus strict schema-v10 storage foundation; writes/query/UI planned | bounded IDs, ppm ratios, optional units, deterministic identities, reset/allowance evaluation, exact v9 rollback migration, strict quota objects, same-window evidence FKs, malformed-reopen and allowance-direction contracts pass; transactional writes/retention/read/query/provider/UI pending |
...
| TM-DATA-008 | partial | exact quota values/evaluator plus strict schema-v10 quota storage foundation | constructors and detector reject invalid/mismatched state; exact v9 migration seeds empty quota revision without usage/price mutation; strict tables enforce bounded fields, exact same-window evidence, semantic allowance changes, malformed reopen, and atomic rollback; writes/retention/read contracts pending |
...
| TM-SEC-002 | partial | current JSONL/store boundaries plus exact quota domain, detector, and strict quota schema validate types, sizes, evidence, and state continuity | quota values/evaluator reject invalid or inferred state; schema-v10 rejects cross-window evidence, incomplete allowance facts, weakened schema, and migration residue; future provider/config/CLI/MCP/plugin boundary suites pending |
...
P2-D Tasks 1-3 exact quota values, pure detector, and strict schema-v10 foundation are
implemented; transactional history writes, retention, query, transport, and reset
