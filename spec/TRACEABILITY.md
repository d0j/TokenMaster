...
| TM-FUNC-012 | partial | P3-D.0 library-only `tokenmaster-state` foundation plus future store/platform/app integration | Task 1 stable error/bound values and dependency/authority contracts pass; settings/package/snapshot/retention/recovery/safe-mode evidence pending |
...
| TM-DATA-011 | planned | redundant records, fixed typed packages, self-describing restore points, and durable recovery journal | A/B corruption, package vector, retention, journal crash-phase, quarantine, and compatibility gates pending |
...
| TM-SEC-008 | partial | library-only state authority boundary plus future strict package validation, fixed writer identity, staged replacement, and quarantine | Task 1 checks exact workspace/dependency identity, no bin/build target, no standard-library/platform alias, macro, filesystem/process/network/shell/SQL/UI/archive/external-source authority, no public path constructor, and no forbidden transitive dependency across 29 mutations; package/recovery/adversarial gates pending |
