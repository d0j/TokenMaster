...
| TM-FUNC-012 | partial | prior reliable-state stack plus Task 12A, Task 12B.1 command/restart, Task 12B.2a selected restore, and Task 12B.2b.1 worker/config core | prior evidence plus one joined capacity-one operation worker, real off-UI manual backup binding, 2 MiB typed config export/read/preview/atomic-commit contracts, current-directory restore pinning, and fail-closed lifecycle contracts pass; native-file/UI config binding, verify/restore/rebuild commands, no-backup reconstruction, health, and UI remain |
...
| TM-DATA-011 | partial | prior reliable-state layers plus Task 12A, Task 12B.1, and Task 12B.2a selected restore composition | prior evidence plus opaque package identity across catalog reorder, admitted-before-pin retention protection, journaled selected replacement, accepted recovery generation, and repeated legacy migration safety; worker/UI bindings, no-backup reconstruction, and release resource gates pending |
...
| TM-SEC-008 | partial | prior sealed state plus Task 12A, Task 12B.1, and Task 12B.2a selected restore gate | prior evidence plus path/digest-private identity binding, deletion-serialized late pin, protected safety point, receipt-before-restart ordering, restored-legacy migration replay, old-owner join/fresh guard, and source-policy mutations; rebuild, external bindings, UI, reconstruction, and release gates pending |
