...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, app-owned in-app expiry presentation, and global reminder settings editor | global portable desired state synchronizes generation-bound global reminders through startup, Save, and confirmed import; per-scope editing, OS/tray, usage alerts, and P3-E remain |
...
| TM-FUNC-010 | partial | prior benefit/reminder stack, expiry-safety center, app-owned in-app presentation receipts, and global reminder settings synchronization | portable settings map generation `N` to global revision `N + 1`; inherited due rebuild preserves overrides and delivery/ack/provider evidence, while snooze, quiet hours, OS delivery, and activation remain |
