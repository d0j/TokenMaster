...
| TM-FUNC-009 | partial | exact quota domain/evaluator plus strict schema-v10 transactional storage; read/query/UI planned | bounded IDs, ppm ratios, optional units, deterministic identities, reset/allowance evaluation, exact v9 rollback migration, immutable transactional writes, duplicate/stale no-op, repeated reset/reopen/account isolation, five-boundary rollback, and current-projection fail-closed contracts pass; retention/read/query/provider/UI pending |
...
| TM-DATA-008 | partial | exact quota values/evaluator plus strict schema-v10 transactional quota storage | constructors and detector reject invalid/mismatched state; exact v9 migration seeds empty quota revision without usage/price mutation; strict tables and the immediate writer enforce bounded fields, exact same-window evidence, global observation identity, immutable definitions, semantic allowance changes, exact current projection, revision/sequence bounds, five-boundary rollback, and deterministic retry; retention/read contracts pending |
...
| TM-SEC-002 | partial | current JSONL/store boundaries plus exact quota domain, detector, strict quota schema, and transactional writer validate types, sizes, evidence, identity, and state continuity | quota values/evaluator reject invalid or inferred state; schema/write path rejects cross-window evidence, incomplete allowance facts, observation/definition reuse, missing current projection, weakened schema, migration residue, and partial publication; future provider/config/CLI/MCP/plugin boundary suites pending |
...
P2-D Tasks 1-4 exact quota values, pure detector, strict schema-v10 foundation, and
transactional history writes are implemented; retention, query, transport, and reset
inventory remain. P3 UI evidence remains separate.
M0 interactive/
