...
| TM-REL-003 | partial | P3-D.7 standard pinned Slint attribution surface plus P6 explicit MSVC signed portable package and supply-chain gates | in-product attribution exists; the locked GNU production binary builds and starts from an isolated portable stage, but the host lacks the MSVC compiler, Windows SDK/signing tools, and canonical package provenance. GNU/MSVC comparison, generated notices/SBOM, public-download attribution, advisory/source/license/secret/action/attestation audits, deterministic package and accepted clean-room launch remain pending |
