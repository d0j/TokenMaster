...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, P3-E.1 route command palette, P3-E.2 compact quota mode, P3-E.3 typed tray lifecycle, P3-E.4 current-session activation, P3-E.5 current-user startup, app-owned in-app expiry presentation, global reminder settings editor, and P4-E durable layout axis | P4-E visibly switches Refined/Control Center/Workbench over existing six sections with narrow single-column fallback and durable selection; board reorder/hide/collapse, per-scope editing, reminder OS/tray delivery, usage alerts, and remaining presentation work remain |
| TM-FUNC-005 | partial | separate `crates/desktop` frontend, sole `crates/app` production binary, `tokenmaster-platform` current-session/current-user startup owners, and P4-E complete four-axis presentation owner | P4-E persists schema-v5 density+skin+System/Light/Dark plus Refined/Control Center/Workbench, migrates v1-v4 to Refined, retains one active plus one latest complete payload, and proves 81 combinations plus 10,000 switches. Locale/language, typography/row sizing, accessibility/DPI/paint/resource verification, and live Windows acceptance remain P4/P6 |
...
| TM-UI-002 | partial | immutable publication, bounded routes/palette/compact projection, independent notification epoch bridge, and P4-E complete presentation owner | P4-E fixes four keys/indices for density, skin, requested color scheme, and layout; schema-v5 complete persistence; v1-v4 Refined migration; admission-before-apply; one owner; compiled UI 81-combination and 10,000-switch proofs without new authority. Locale, typography/row-size behavior, accessibility/DPI/paint/resource verification, and hot locale settings remain P4 |
...
| TM-DATA-006 | partial | prior bounded publication plus current data projections, replace-only P3-D.2c Sessions navigation, interactive History range/rhythm envelope, static Help/About, and P3-E.2 compact projection | prior caps remain; History range selection replaces one shared at-most-30-day envelope through one worker/slot, including fixed 24+7 rhythm models capped at 768 occurrences/2,304 segments; Sessions retains one at-most-64 page and no cursor history, while Compact reuses the current at-most-32 quota rows; P4 switch limits remain |
