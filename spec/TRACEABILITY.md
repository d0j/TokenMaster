...
| TM-FUNC-012 | partial | P3-D.0 typed settings, store-owned verified SQLite candidates, fixed typed packages, manual age protection, and bounded verified restore-point retention over sealed platform capabilities | Tasks 1-8 pass stable authority/settings, WAL-consistent Online Backup, deterministic `.tmconfig`/`.tmbackup`, verified-source-bound age v1 export/import, exact prepublication verification, 32-slot disposable catalog, 15-point/byte-bounded retention, full-set pre-delete revalidation, and crash-visible one-file deletion; maintenance/recovery/safe-mode evidence pending |
...
| TM-PERF-004 | partial | bounded record writer, page-stepped snapshot verifier, and fixed streaming package codec plus future capacity-one maintenance worker | record save writes at most 256 KiB per call; snapshot uses 64-page steps and fixed verification limits; package I/O uses 64 KiB buffers, 256 KiB maximum durable chunks, 8 MiB decoder window, independent expanded counter, and a 24 MiB streaming contract; maintenance latency, sleep/resume, and repeated resource gates pending |
...
| TM-DATA-011 | partial | controlled durable files/readers, redundant records, typed settings, verified SQLite candidates, fixed v1 typed packages, and disposable bounded catalog/retention plus future journal | prior evidence plus exact package vectors, strict single-frame Zstd, sealed-stage verification, exact 32-slot namespace, cold HeaderValid/Corrupt truth, current proof binding, deterministic 4+7+4 UTC tiers, shared 15-point/byte cap, and one-delete rebuild/replan pass; journal transitions, quarantine, and future compatibility gates pending |
...
| TM-SEC-008 | partial | prior state/platform authority, fixed store snapshots, controlled streaming package codec, sealed backup directory/catalog/retention, and future quarantine | prior evidence plus no main-only backup/caller SQL/path, minimal-feature Zstd/age, irreversible stage poison, fixed private slots, link/hardlink/identity rejection, prepublication typed verification, no-delete admission, full verified-set/target rehash, write-through tombstone deletion, private errors/Debug, and 42 authority mutations; recovery gates pending |
