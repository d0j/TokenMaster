...
| TM-FUNC-009 | partial | exact quota domain/evaluator plus strict schema-v10 transactional storage, bounded retention, and defensive store reads; public query/provider/UI planned | bounded IDs, ppm ratios, optional units, deterministic identities, reset/allowance evaluation, exact v9 rollback migration, immutable transactional writes, evidence-preserving retention/hard caps, exact 32-window current snapshots, revision-bound 256+1 keyset transition pages, owned boundary values, deadline cleanup, and projection/over-cap/post-open-drift fail-closed contracts pass; public query/provider/UI pending |
...
| TM-PERF-001 | partial | bounded parser/reader/store/engine plus live runtime and query facade | unchanged payload bytes=0; exact tail; 300-event batches; 10,000-hint aggregate/one follow-up; one retained result across 10,000 candidates; 10,000 equivalent quota polls plateau at protected first/latest samples; quota reads cap current keys at 32 and transition work at 256+1 indexed rows with a two-second total deadline; watcher/live/power baselines and 256-cycle query resource plateau pass; quota/UI/plugin release evidence pending |
...
| TM-DATA-008 | partial | exact quota values/evaluator plus strict schema-v10 transactional storage, bounded retention, and defensive reads | prior write/retention contracts plus 32-key current capture, revision-bound 256+1 descending keyset history, exact owned definition/sample/epoch/transition restoration, deterministic transition-ID verification, current/boundary projection reconciliation, stale/filter/deadline cleanup, quota-only index plans, and post-open drift rejection pass; public query/provider/inventory/UI remain |
...
| TM-SEC-002 | partial | current JSONL/store boundaries plus exact quota domain, detector, strict quota schema, transactional writer, fixed retention, and defensive quota reader validate types, sizes, evidence, identity, and state continuity | prior quota gates plus fixed query-only SQL, revision/filter-bound cursor, deterministic transition restore, current/boundary sample reconciliation, total deadline, handler cleanup, and post-open projection drift rejection pass; future provider/config/CLI/MCP/plugin boundary suites pending |
| TM-SEC-003 | implemented | provider/Codex/store/engine errors, value types, redacted worker panic boundary, and redacted quota identities/captures | serialized/debug privacy, path-redaction, sealed identity, path-substitution, raw-archive-write compile-fail, redacted observation/scope/epoch/transition/cursor/filter output, stable quota error-code, fixed panic/fault completion, and panic-strategy compile guard contracts |
...
P2-D Tasks 1-6 exact quota values, pure detector, strict schema-v10 foundation,
transactional history writes, evidence-preserving bounded retention, and defensive
store snapshots/keyset history are implemented; public query, transport, and reset
inventory remain. P3 UI
