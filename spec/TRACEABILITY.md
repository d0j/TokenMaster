...
| TM-FUNC-012 | partial | prior reliable-state stack plus Task 12A state owner and Task 12B.1 command/restart core | prior evidence plus one-active/one-follow-up typed admission, 10,000-hint coalescing, exact cancellation/irreversible boundary, fresh-guard current-bundle restart, and obsolete-notifier rejection pass; operation worker, actual restore/import/export/rebuild, no-backup reconstruction, health, and UI remain |
...
| TM-PERF-004 | partial | bounded reliable-state path plus backup maintenance and constant-state app command admission | prior fixed bounds plus backup worker/scheduler, one command active/follow-up under 10,000 hints, bounded staging/quarantine, worker-only catalog verification, retained projection, atomic waits, and the 36-cycle Windows maintenance resource gate pass; operation-worker and restore/restart resource evidence remain |
...
| TM-DATA-011 | partial | prior reliable-state layers plus Task 12A backup/migration and Task 12B.1 command/restart composition | prior evidence plus durable pending migration, complete snapshot-to-retention, bounded typed selection/admission, same-window fresh-guard bundle replacement, generation-safe notifiers, and clean-after-join; selected restore, no-backup reconstruction, UI, and release resource gates pending |
...
| TM-SEC-008 | partial | prior sealed state plus Task 12A lifecycle and Task 12B.1 bounded command/restart gate | prior evidence plus safe-shell isolation, sealed backup, durable migration ordering, exact-root waits, path-free fixed command values, one irreversible cancel boundary, old-owner join/fresh guard, and same-lock obsolete-generation rejection; selected restore, rebuild, UI, and release gates pending |
