...
| TM-FUNC-005 | partial | separate `crates/desktop` frontend, sole `crates/app` production binary, `tokenmaster-platform` current-session/current-user startup owners, and P4-D complete presentation owner | P4-D persists schema-v4 density+skin+System/Light/Dark, migrates v1-v3 to legacy Dark, binds typed package source versions, applies one of six exact Rust-owned 15-role palettes before metadata, admits before apply, retains one active plus one latest complete payload, and proves 10,000 27-combination switches. Layouts/locales, typography/row sizing, accessibility/DPI/paint/resource verification, and live Windows acceptance remain P4/P6 |
...
| TM-UI-002 | partial | immutable publication, bounded routes/palette/compact projection, independent notification epoch bridge, and P4-D complete presentation owner | P4-D fixes three keys/indices for each of density, skin, and requested color scheme; schema-v4 complete-triple persistence; legacy Dark migration; saved/saving/not-saved semantics; seven density token tables; six exact 15-role palettes; reactive effective System observation without revision/persistence churn; admission-before-apply; one owner; and exact application plus compiled-UI 10,000-cycle proofs without new authority. Layouts/locales, typography/row-size behavior, accessibility/DPI/paint/resource verification, and hot locale/layout settings remain P4 |
