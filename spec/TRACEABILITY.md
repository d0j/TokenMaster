...
| TM-PERF-003 | partial | immutable engine publication, complete P2-A activity query, and P2-B transactional aggregate storage/rebuild/read | prior cursor/query evidence plus schema-v8 exact triggers, bounded resumable publication, measured 1/32/256 append p95 of 1.814/19.888/230.620 ms, and exact fixed overview over at most three UTC segments/32 scopes pass; series/breakdown/session reads, million-row dashboard/resource evidence, and P3 UI remain |
...
| TM-DATA-006 | partial | reader/parser/store, engine/runtime, P2-A query, and P2-B rebuild/read bounds | prior limits plus 256+1 lookahead, 256-event/2304-rollup cleanup pages, fixed overview with three adjacent UTC segments/32 unique scopes, 16 warnings, exact read transactions and two-second maximum; series/breakdown/session public values and UI/plugin limits pending |
