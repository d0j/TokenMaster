...
| TM-REL-001 | partial | M0 scripts, serial Windows GNU Cargo policy, 60-minute M0 CI receipt budget, latest-only terminal-completion controller contract, deterministic product producer/validator, product receipt schemas, separate P3-D.0 developer receipt, and `P3E_ACCEPTANCE.md` | the P6 unsigned producer and P3-E preflight bind one clean commit, closed package/manifest, producer receipt, packaged/tested executable, and operator receipt; M0 serializes its GNU linker work, grants sufficient clean-run CI wall time, and correctly waits for terminal worker state rather than a count of lossy latest receipts. Authenticated external receipts, current schema-4 P3-D.0 exact-commit receipt, signing, and final product acceptance remain pending |
