...
| TM-FUNC-009 | partial | exact quota domain values and pure immutable-epoch/reset evaluation plus planned storage/query/UI | bounded IDs, ppm ratios, optional units, definitions/samples, deterministic identities, duplicate/stale/conflict gates, scheduled/early/repeated/manual reset, reset+allowance, sequence/restart, and rolling/drop rejection contracts pass; schema/retention/read/query/provider/UI fixtures pending |
...
| TM-DATA-008 | partial | exact normalized quota definitions/samples plus pure epoch, reset, and allowance evaluation | constructors reject invalid IDs/ppm/time/capacity/evidence/serde; detector proves deterministic scope/epoch/transition identities, restart-safe definition revisions, monotonic sequences, maximum comparable use, evidence precedence, reset classification, and fail-closed mismatches; schema/retention/read contracts pending |
...
| TM-SEC-002 | partial | current JSONL/store boundaries plus exact quota domain and pure detector validate types, sizes, evidence, and state continuity | quota values reject invalid/ambiguous serde; detector rejects mismatched/conflicting/regressed/overflow state and untrusted threshold inference; future provider/config/CLI/MCP/plugin boundary suites pending |
