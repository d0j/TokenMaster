...
| TM-FUNC-012 | partial | prior reliable-state stack plus Task 12A, Task 12B.1 command/restart, and Task 12B.2a selected restore | prior evidence plus current-directory identity binding, deletion-serialized RAII pin, protected PreRestore, joined journaled restore, run-session receipt acceptance, restored-legacy migration gates, and stale/shutdown fail-closed contracts pass; operation worker, UI binding, import/export/verify/rebuild, no-backup reconstruction, health, and UI remain |
...
| TM-PERF-004 | partial | bounded reliable-state path plus backup maintenance, constant-state app command admission, and immutable catalog snapshots | prior fixed bounds plus backup worker/scheduler, one command active/follow-up under 10,000 hints, bounded staging/quarantine, short-held catalog mutex, worker-only verification, atomic waits, and the 36-cycle Windows maintenance resource gate pass; operation-worker and selected-restore release resource evidence remain |
