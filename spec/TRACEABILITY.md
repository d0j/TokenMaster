...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, P3-E.1 route command palette, P3-E.2 compact quota mode, P3-E.3 typed tray lifecycle, app-owned in-app expiry presentation, and global reminder settings editor | palette and compact reuse existing route/quota truth; one five-intent tray router reaches only the same window and joined shutdown; global portable desired state synchronizes reminders through startup, Save, and confirmed import; per-scope editing, reminder OS/tray delivery, usage alerts, and remaining P3-E remain |
| TM-FUNC-005 | partial | `crates/probe-app` evidence plus separate `crates/desktop` frontend and sole `crates/app` production binary | software-only app composition and same-window P3-E.1/P3-E.2/P3-E.3 palette, compact, tray/close lifecycle pass source/release gates; M0 evidence remains separate and hotkey/single-instance/startup plus interactive Explorer/focus/resource closure remain P3-E/P6 |
...
| TM-UI-001 | partial | complete bounded data routes, static Help/About, P3-E.1 command palette, P3-E.2 compact quota mode, P3-E.3 tray lifecycle, and transient accessible expiry panel | real Slint proves palette/compact activation, explicit unknown ratio, exact size restore, and exact five-action tray callback mapping; unified localization and full interactive tray/focus/accessibility acceptance remain P4/P6 |
...
| TM-PERF-001 | partial | bounded backend, one-current projections, capped P3-E.1/P3-E.2 models, queue-free P3-E.3 lifecycle router, one notification receipt worker, and constant-capacity bridges | palette/compact/notification stress bounds remain; tray adds one component, one optional retained handle wrapper, one router slot, zero queue/timer/worker/cache/controller owners; interactive cycle/resource and long-run P4/P6 evidence remain |
