...
| TM-REL-001 | partial | M0 scripts, deterministic product producer/validator, product receipt schemas, separate P3-D.0 developer receipt, and `P3E_ACCEPTANCE.md` | the P6 unsigned producer and P3-E preflight bind one clean commit, closed package/manifest, producer receipt, packaged/tested executable, and operator receipt; authenticated external receipts, current schema-4 P3-D.0 exact-commit receipt, signing, and final product acceptance remain pending |
...
| TM-REL-003 | partial | P3-D.7 standard pinned Slint attribution surface plus P6 explicit MSVC signed portable package and supply-chain gates | in-product attribution exists; the canonical MSVC binary and deterministic unsigned nine-file ZIP pass exact target/content/hash validation with generated notices/license texts and CycloneDX SBOM. Public-download attribution, signing, advisory/source/license/secret/action/attestation audits, authenticated clean-room launch, interactive matrix, performance comparison, and soak remain pending |
