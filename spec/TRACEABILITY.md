...
| TM-FUNC-012 | partial | prior reliable-state stack plus completed Task 12 application lifecycle, Task 14 sealed file selection, Task 15 Data & Recovery UI, and Task 16 adversarial closure | prior evidence plus exhaustive package prefix/one-bit rejection, coherent/resumable WAL/SHM layout preflight, a 57-test executable app recovery aggregate, exact dependency/license/feature identities, privacy/release-binary audit, and application/state authority mutations pass; Task 17 resource and Task 18 acceptance closure remain |
...
| TM-DATA-011 | partial | complete reliable-state data/application/UI contour through Task 16 | prior evidence plus exhaustive package mutation/truncation rejection, coherent main/active/quarantined-WAL/SHM preflight, retained per-move identity checks, exact interrupted-move resume, executable app/state recovery matrix, and privacy-safe package/license audit pass; Task 17-18 resource/acceptance closure remains |
...
| TM-SEC-008 | partial | sealed reliable-state layers plus complete application/UI command, reconstruction, and Task 16 adversarial contour | prior evidence plus every package prefix/one-bit mutation, WAL/SHM drift/conflicting-target no-new-partial-move proof, exact partial resume, twenty-three executable attack anchors, SHA-pinned dependency/features/licenses, production/export/release privacy canaries, and zero codec process/network/shell/extraction/plugin/UI/SQL authority; Unix cleanup hardening plus Task 17-18 resource/release gates remain |
