...
| TM-FUNC-012 | partial | P3-D.0 typed settings, verified SQLite candidates, fixed typed packages, bounded retention/maintenance, sealed journaled restore, and pre-open state bootstrap | Tasks 1-11A add stable A/B run truth, unclean-before-open publication, read-only normal/quick diagnosis, pending-journal-first resume, newest-first reverified corruption-only data-only recovery, two-launch retry bound, same-guard live-runtime handoff, and first-install/damaged-missing distinction to the prior Tasks 1-10 evidence; application-owned no-backup reconstruction, migration safety points, safe mode, commands, and UI remain Task 12+ |
...
| TM-DATA-011 | partial | controlled files/records/settings/run state, verified SQLite candidates, typed v1 packages, catalog/retention, constant-state maintenance, six-phase restore journal/quarantine, and pre-open bootstrap | prior evidence plus exact run generation/digest CAS, clean/unclean/missing/invalid classification, bounded quick check, recovery-operation launch tracking, historical-complete acceptance, zero-length sidecar facts, cold journal resume, newest-first skip, and no-backup evidence preservation; application composition/UI and release resource gates pending |
...
| TM-SEC-008 | partial | prior state/platform authority, fixed store snapshots, controlled codec/catalog/retention, capacity-one maintenance, sealed restore/quarantine, and root-bound startup bootstrap | prior evidence plus cross-root rejection before mutation, strict post-marker staging scan, unclean-before-open, read-only non-migrating diagnosis, non-corruption preservation, same-guard runtime adoption, recovered-candidate retry bound, and exact clean publication capability; application lifecycle/migration/rebuild/UI recovery gates pending |
