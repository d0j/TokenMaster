...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, P3-E.1 route command palette, P3-E.2 compact quota mode, app-owned in-app expiry presentation, and global reminder settings editor | the route-only palette reuses the 11-route projection; compact mode reuses every current bounded quota row with explicit unknown/unavailable truth; global portable desired state synchronizes reminders through startup, Save, and confirmed import; per-scope editing, OS/tray, usage alerts, and remaining P3-E remain |
| TM-FUNC-005 | partial | `crates/probe-app` evidence plus separate `crates/desktop` frontend and sole `crates/app` production binary | software-only app composition, same-window P3-E.1 palette, and same-window P3-E.2 compact quota mode pass; M0 evidence remains separate and production tray/hotkey/single-instance/startup/close lifecycle remain P3-E/P6 |
...
| TM-UI-001 | partial | complete bounded data routes, static Help/About, P3-E.1 command palette, P3-E.2 compact quota mode, and transient accessible expiry panel | real headless Slint proves palette plus compact keyboard/pointer/accessibility route activation, explicit unknown ratio, exact size restore, and all notification rows applied before visibility receipt; unified localization and full interactive accessibility acceptance remain P4/P6 |
...
| TM-PERF-001 | partial | bounded backend, one-current projections, capped data models including P3-E.1/P3-E.2, one notification receipt worker, and constant-capacity bridges | a 10,000-scalar palette input retains 64 scalars and at most 11 rows; 10,000 compact/dashboard switches retain the same component, quota model and one geometry slot; 10,000 notification pumps retain one lease/presentation and one transient 256-row model; long-run P4/P6 resource evidence remains |
