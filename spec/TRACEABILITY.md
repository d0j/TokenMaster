...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, fixed route readiness, P3-A shell, P3-B.1 controller, P3-B.2 event delivery, and P3-B.3 live application composition; route payloads remain | prior gates plus deterministic installed/portable root, sole production binary/runtime owners, fixed runtime-health join, real live-bundle shutdown, 21 adversarial Pester cases, desktop/app release audits, and the complete strict workspace gate; P3-C-P3-E remain |
...
| TM-UI-002 | partial | immutable runtime/query/product publication, P3-A projection, P3-B.1 controller snapshot, P3-B.2 Slint delivery, and P3-B.3 one-current runtime-health observation | prior gates plus lossy worker hints, checked runtime generations, 10,000 newest-only observations, active-query follow-up, one weak app notifier, and zero polling; P4 switches remain |
| TM-PERF-001 | partial | bounded backend, one-current product/desktop projection, one query worker/snapshot slot, one runtime-observation slot, and one event gate | prior gates plus 1,000 query hints, 10,000 runtime observations, 10,000 UI notifications, no notifier dispatcher/timer/queue, and exact composition audit; UI switch/resource and long-run release evidence remain P4/P6 |
...
| TM-DATA-001 | partial | prior privacy boundaries plus deterministic path-private app root and copied count-only runtime health | P3-B.3 errors/Debug and release binary expose no archive/Codex/private fixture/old-project data; P3-C-P5 must repeat gates |
...
| TM-DATA-006 | partial | bounded backend, one-current product/route model, one query/snapshot slot, one fixed runtime-observation slot, and shared event bridge | prior limits plus 10,000 newest runtime observations, four copied health results, checked generation, lossy weak hints, and one-owner composition audit; paged route payload and P4 switch limits remain |
...
| TM-SEC-002 | partial | prior strict boundaries plus P3-B.3 separate app/frontend authority and weak runtime-health delivery | one binary/runtime/controller/bridge owner, invalid-marker fail-closed, receipt-before-hint panic isolation, no desktop runtime authority, no path/private binary strings, and 21 adversarial cases pass; P3-C-P5/plugin suites pending |
