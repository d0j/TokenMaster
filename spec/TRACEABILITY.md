...
| TM-FUNC-004 | partial | exact joined product status, immutable product reducer, fixed route readiness, P3-A production Slint shell, and P3-B.1 bounded query controller; route payloads remain | prior gates plus status-first typed reduction, section-local failure, real schema-v13 read, 1,000-hint coalescing, latest-only snapshot, cancellation/deadline/shutdown/redaction and desktop audit pass; P3-B.2-P3-E remain |
...
| TM-UI-002 | partial | immutable runtime/query/product publication, P3-A one-current projection, and P3-B.1 one-current controller snapshot | prior gates plus attempt/receipt separation, one reducer, complete-attempt publication, cancellation/deadline discard, and latest-only result pass; P3-B.2 event-loop delivery and P4 switches remain |
| TM-PERF-001 | partial | bounded backend plus one-current product/desktop projections and one desktop query worker/latest slot | prior gates plus 1,000 hints to one follow-up, one worker/slot audit, and no query call on the UI adapter pass; UI switch/resource and long-run release evidence remain P4/P6 |
...
| TM-PERF-003 | partial | immutable engine/product publication, indexed facades, stale-safe desktop projection, and bounded query controller | prior scale evidence plus exact controller attempt generations, coalesced follow-up, section-local failure, and off-UI query ownership; P3-B.2 delivery and P4 visible-paint evidence remain |
...
| TM-DATA-006 | partial | bounded backend, one-current product snapshot/route model, and one-worker/one-latest-slot desktop controller | prior limits plus fixed projection, typed capped requests, one active/one pending refresh, 1,000-hint/latest-only tests, and controller source audit pass; event delivery, paged route payload, and P4 switch limits remain |
...
| TM-SEC-002 | partial | current JSONL/store/query/product boundaries plus strict app-server, quota-benefit, reminder, Git, and desktop-controller surfaces validate type, size, evidence, identity, continuity, deadline, ordering, isolation, and redaction | prior gates plus P3-B.1 typed-only query source, status-first reduction, cancellation/deadline partial-publication rejection, path-free open error, no UI query/direct store-runtime authority, and release-string privacy pass; P3-B.2-P5/plugin suites pending |
