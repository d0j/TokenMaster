...
| TM-FUNC-012 | partial | P3-D.0 typed settings, store-owned verified SQLite candidates, and fixed typed packages over crate-private records and controlled durable files | Tasks 1-6 pass stable authority/settings, WAL-consistent Online Backup, independent verification/compaction/identity, deterministic `.tmconfig`/`.tmbackup`, poisoned failure output, and package privacy/adversarial gates; encryption/retention/recovery/safe-mode evidence pending |
