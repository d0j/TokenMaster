...
| TM-UI-002 | partial | immutable publication, bounded routes/palette/compact projection, independent notification epoch bridge, and P4-A density owner | P4-A fixes Comfortable/Compact/Ultra compact keys and indices, seven exact spacing/radius tables, checked revision, one root binding/callback, and a 10,000-switch test without timer/worker/query/window/retained authority. Full skins/layouts/color schemes/locales, persistence/migration, remaining density typography/row-size behavior, accessibility/DPI/paint/resource verification, and hot skin/locale/layout settings remain P4 |
