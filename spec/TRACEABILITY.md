...
| TM-FUNC-008 | partial | built-in provider-neutral drafts plus constant-state coordinator and bounded synchronous engine ports; one-shot execution, live Codex composition, and plugin host pending | provider/Codex/accounting contracts plus P1-C.1 coordinator and P1-C.2 value/batch/port/compile-fail contracts |
...
| TM-PERF-001 | partial | parser, reader, domain, store bounds, P0-E composition, and P1-C coordinator/ports | 300-file/300-event, 256 observation/relation batches, 256 replay-source pages, 32-KiB checkpoints, 18 chunk updates, reopen, and 10,000-hint constant-state contracts pass; query/plugin runtime bounds pending |
...
| TM-DATA-006 | partial | reader/parser/store plus engine value/batch/page limits | line/resume, 32-KiB opaque checkpoint, 256 batch/page, 18 chunk-update, and SQLite-i64 counter bounds; full UI/query/plugin limits pending |
...
one-shot execution and the worker shell remain P1-C.3/P1-C.4.
