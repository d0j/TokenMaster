...
| TM-FUNC-007 | partial | accounting lineage/classifier, scalable replay archive, P0-E composition, P1-A retention, and active Partial engine publication | synthetic JSONL baseline/append/restart/replay/quality/atomic-replacement/truncation-retention/failure contracts pass; exact parent-index seeks, bounded statement caching, disposition-only classification refresh, and schema-v14 full-key time-rollup maintenance remove measured whole-archive work; runtime snapshots now expose durable Partial truth before worker completion, proven by focused 1,024-event and real 4,007-source receipts, while contract-preserving bulk ingest and complete cold-import evidence remain open |
...
| TM-DATA-005 | implemented | writable usage/quota/benefit/Git store plus separate `UsageReadStore` | strict schema v14 retains all prior facts, atomically replaces four broad time-rollup mutation triggers with full-primary-key seeks, and preserves one exact scalar cross-family product-status transaction with defensive deadline/corruption/progress cleanup and no historical scan |
