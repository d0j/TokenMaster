...
| TM-FUNC-007 | partial | accounting lineage/classifier, scalable replay archive, P0-E composition, and P1-A retention | synthetic JSONL baseline/append/restart/replay/quality/atomic-replacement/truncation-retention/failure contracts pass; exact parent-index seeks, bounded statement caching, disposition-only classification refresh, and schema-v14 full-key time-rollup maintenance remove measured whole-archive work; a 120-second 5.0 GiB local-history receipt advanced 8,121 events and shut down cleanly, while complete cold-import and package evidence remain open |
...
| TM-PERF-003 | partial | immutable publication, bounded query controller, newest-only snapshot bridge, capacity-one Sessions terminal rollback, independent notification epoch bridge, durable-progress recovery hints, and live app hints | current replay uses an exact parent index, fixed-capacity prepared statements, and schema-v14 full-key time-rollup triggers; one measured 256-observation transaction fell from 74,592 ms to 291 ms, a real 120-second copy advanced from 51,530 to 59,651 events, and shutdown completed in 122.30 s; final package/external release evidence remains |
