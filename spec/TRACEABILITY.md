...
| TM-FUNC-004 | partial | exact joined product status, immutable reducer, complete bounded data routes, Help/About, P3-E.1 route command palette, P3-E.2 compact quota mode, P3-E.3 typed tray lifecycle, P3-E.4 current-session activation, P3-E.5 current-user startup, app-owned in-app expiry presentation, global reminder settings editor, P4-E layouts, and P4-F board preferences | P4-F closes board reorder/hide/collapse/reset with strict six-key manifest, all-six payload retention, and compiled evidence; per-scope editing, reminder OS/tray delivery, usage alerts, and remaining presentation work remain |
| TM-FUNC-005 | partial | separate `crates/desktop` frontend, sole `crates/app` production binary, `tokenmaster-platform` current-session/current-user startup owners, and P4-E/P4-F complete presentation owner | P4-F persists schema-v6 four axes plus fixed board manifest, retains one active plus one latest complete payload, and proves board UI 5/5 including 10,000 edits and layout 4/4. Locale/language, typography/row sizing, accessibility/DPI/paint/resource verification, and live Windows acceptance remain P4/P6 |
...
| TM-UI-002 | partial | immutable publication, bounded routes/palette/compact projection, independent notification epoch bridge, and P4-E/P4-F complete presentation owner | P4-F adds schema-v6 fixed board order/visibility/collapse/reset, admission-before-apply, all-six model retention, board UI 5/5 with 10,000 edits, and layout 4/4 without new authority. Locale, typography/row-size behavior, accessibility/DPI/paint/resource verification, and hot locale settings remain P4 |
