...
- **P3 — complete desktop UI (in progress):** P3-A separate production shell/fixed
  ...
  typed package import/export are complete; optional manual age protection is next,
  followed by bounded retention,
...
navigation without model/window reconstruction. P3-D.0 Reliable State Tasks 1-6 are
complete and Task 7 is next, then the remaining P3-D supporting data views.
...
and future migrations do not ship before their durability contract. Tasks 1-6
...
streaming `.tmconfig`/`.tmbackup` packages. Encryption, catalog/retention,
maintenance, and recovery remain pending. Task 8/9 now explicitly require sealed
