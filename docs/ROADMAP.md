...
The next successful synchronization makes one bounded same-session restart attempt
only for that missing startup-contended runtime; it adds no polling or retry loop.
Per-scope editing, snooze, quiet hours, reminder OS/tray delivery, usage
alerts, activation, P4/P5/P6, M0 acceptance, package/signing/soak, and release remain
open.

P3-E.1 command palette is complete as developer evidence: one final full-window
route-only overlay reuses the fixed 11-route projection, caps its Unicode query at 64
scalars and its replace-only model at 11 rows, supports keyboard/pointer/accessibility
activation, and refreshes safely while open. P3-E.2 compact content is also complete as
...
coalesces 10,000 requests, and joins before clean publication without a payload, queue,
  ...
  composition audits pass. `P3E_ACCEPTANCE.md` fixes a strict packaged-production
  operator-receipt contract and schema/local preflight; authenticated package provenance
  plus interactive sign-in/relocation/ACL/resource proof remains external P6-dependent
  evidence and is not a pre-P6 implementation slice.
...
P3-A adds the separate software-only `tokenmaster-desktop` package, one current
...
independently verifiable restore target, plus verified SQLite candidates and strict
streaming `.tmconfig`/`.tmbackup` packages and verified-source-bound standard age v1
manual protection with fixed scrypt and zeroizing passphrase boundaries, plus the
