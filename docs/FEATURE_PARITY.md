...
| ccusage: source handling | Codex history discovery, active/archive precedence, incremental update | M1 discovery, bounded reader, exact-scan tail refresh, rebuild fallback, and bounded pathless scheduling implemented; live lifecycle assembly pending |
...
| TokenMaster improvement | Bounded memory, path-private storage, transactional SQLite, cross-process writer safety, immutable UI snapshots | Core contracts, current-generation ingest, portable OS writer lease, and fixed watcher/scheduler state implemented |
