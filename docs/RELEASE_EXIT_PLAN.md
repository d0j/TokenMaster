...
| post-tag | **The localization contract checks one direction only, and a shipped string is already through the hole.** It asserts that every entry of its closed msgid arrays appears as `@tr(...)` in the Slint tree. There is no assertion the other way -- that every `@tr` in the tree is in an array -- so a label added to a view and forgotten in the catalog passes. One already has: `@tr("Alerts")` renders in the shell tab strip while `msgid "Alerts"` exists in neither `translations/ru` nor any contract array, which means it ships as English in Russian today. The fix that matters is the reverse assertion, because it retires the whole class; the one missing string is then just its first catch. | `crates/desktop/tests/localization_contract.rs`; `@tr("Alerts")` in `crates/desktop/ui/main.slint` |
...
| post-tag | **Eight Activity labels ship untranslated in every locale, and the localization contract cannot see them.** `activity_label` in `crates/desktop/src/ui.rs` returns `Read`, `Edit / Write`, `Search`, `Git`, `Build / Test`, `Web`, `Subagents` and `Terminal` as bare `&'static str` from Rust. The contract asserts a closed msgid set and that each entry appears as `@tr(...)` inside the Slint tree -- so a string emitted from Rust escapes it **by construction**, not by oversight. That is the same shape as three other findings this cycle: a gate of which only a third was run, an acceptance script that could not fail on its own clause, and a resource check that fails for reasons unrelated to the code. The fix is two-sided: move these through `@tr` like every other label, and give the contract a rule that catches a user-visible literal handed to a Slint property from Rust, or the next one lands the same way. | `fn activity_label` in `crates/desktop/src/ui.rs`; the closed-set assertions in `crates/desktop/tests/localization_contract.rs` |
