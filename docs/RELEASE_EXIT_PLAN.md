...
| P6 | **The cold-start cost is bounded and belongs to ingestion, not to idling.** Profiled by thread name on the release build: `tokenmaster-refresh` takes 76.3% of a core averaged over 92 seconds, `main` 4.8%, everything else nil. The process draws about 184% of a core for the first 43 seconds and then falls silent -- 197 consecutive one-second samples at zero. So it is the initial scan doing its job, and P6's criterion of a 300 s median has wide headroom. **Earlier entries here called this an idle burn and a release blocker. That was wrong twice over**: the samples landed inside the import window, and the instrument was narrower than the subject -- it followed a single thread while two carry that name, and read the sleeping one. What is still worth asking is whether every launch redoes this or only the first; every launch measured today did. | `fn run_worker`, `crates/engine/src/worker.rs` |
