...
| **P3 blocker** | **Every pass re-UPDATEs 4016 source rows to the value they already hold.** Queried read-only against the live 816 MB archive: one replay revision, 4016 rows in `usage_replay_source`, 4016 sources, 129,157 events, and every single row already `state = 'complete'` — nothing pending, the work is finished. Yet the write-ahead log grows about 16 MB every ten seconds while the database itself gains 30 KB in two minutes, and `tokenmaster-refresh` holds 79.1% of a core summed over every thread of that name. The arithmetic closes exactly: 4016 rows times a 4 KB page is 16 MB, and SQLite does not compare a value before writing its page. So `UPDATE usage_replay_source SET state = ?1` re-writes `'complete'` over `'complete'`, once per source, once per pass. Cost grows with the number of Codex session files — 4024 here, 11 GB — not with new data, so it does not scale. The guard needs care: the call site treats an update count other than 1 as stale state, so skipping the no-op has to distinguish a row that is already correct from a row that is missing. | `fn complete_current_replay_source` in `crates/store/src/usage/replay.rs` |
