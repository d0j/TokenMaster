...
| **P4 blocker** | **The Today card fills with real numbers and then goes back to claiming no evidence, while ingestion is still running.** Measured on a fresh extraction of the shipped package, which starts with an empty archive and imports live: at launch `29,335,964 all time` with `Tokens —`; after 60 seconds `4,455,442,243` with `Tokens 187,163,328` and `Events 1,341 events`; after 120 seconds `11,627,083,850` with both dashes; after 180 seconds `11,833,387,747`, still dashes. The lifetime total climbs correctly throughout, so the archive is fine and the projection is not. **Cause found, and the obvious repair measured and reverted.** `invalidate_if` replaces a superseded section outright while the failure path beside it retains, so a dataset change erases what a query error would have kept; during a first import the identity changes every pass. Making it retain looked right and was worse: the retained payload is the *earliest* one, so the card then read `Events 0 events` at 120 and 180 seconds and held it -- a false zero in place of an honest dash, which is the one thing this product must not do. Whatever the repair is, it has to keep the *latest* proven answer rather than the first, or keep blanking. | `fn invalidate_if` in `crates/product/src/reducer.rs`; reverted in `8d0bd8d` |
