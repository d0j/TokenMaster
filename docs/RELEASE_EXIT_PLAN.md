...
| **P4 blocker** | **The Today card fills with real numbers and then goes back to claiming no evidence, while ingestion is still running.** Measured on a fresh extraction of the shipped package, which starts with an empty archive and imports live. Header and Today at four moments: at launch `29,335,964 all time` with `Tokens —` and `Events 0 events`; after 60 seconds `4,455,442,243 all time` with `Tokens 187,163,328` and `Events 1,341 events`; after 120 seconds `11,627,083,850 all time` with `Tokens —` and `Events —`; after 180 seconds `11,833,387,747 all time`, still both dashes. The lifetime total keeps climbing correctly throughout, so the archive is fine and the projection is not -- a first-time user watches their own numbers appear and then vanish. Suspected but not measured: the reducer invalidates sections whose dataset identity no longer matches, and during a first import that identity changes constantly, so analytics may be invalidated faster than it is republished. | `fn invalidate_incompatible_sections` in `crates/product/src/reducer.rs`; `fn map_analytics` in `crates/desktop/src/dashboard.rs` |
| **P4 blocker** | **The quota board can never work on a Windows machine where Codex was installed with npm, which is the usual way.** Quota is not read from the archive; the runtime shells out to the Codex executable, and discovery looks for exactly one file name: `codex.exe` on Windows, `codex` elsewhere. Measured on this machine: `codex` resolves through PATH to the npm shims `codex.cmd` and `codex.ps1` under `AppData/Roaming/npm`, and **no `codex.exe` exists anywhere on PATH** -- an npm install ships shims, not a native executable. So discovery returns `Unavailable`, every quota table stays empty (`quota_sample` 0, `quota_window_current` 0), and the board reads `Quota evidence unavailable` forever. The stranger test sees the same. This is the working-quota-board half of the P4 criterion. The fix has to try the shim names too, and spawning a `.cmd` is not the same as spawning an `.exe` -- that needs checking, not assuming. | `fn native_executable_name` in `crates/runtime/src/quota/discovery.rs` |
