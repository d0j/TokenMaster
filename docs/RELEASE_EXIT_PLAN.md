...
| **P3** | **The board's scrolled content is at least twice as wide as the viewport, so a whole metric is unreachable.** Measured, not inferred: a temporary marker row of `red(stretch 1) | green(40px) | blue(stretch 1)` was placed as the first child of the `ScrollView` content and the window captured at 1400, 1900 and 2400 physical pixels with `SLINT_SCALE_FACTOR=1`. At every width the red half alone fills the entire visible width -- pure `255,0,0` out to the last pixel -- and neither the green centre marker nor the blue half is on screen, which puts the content at 2.0x the viewport or more, stable across all three widths. So it scales with the window rather than being a fixed minimum. The visible cost: the Today card's `EVENTS` column is never rendered at any window size, and `TOKENS` is cut mid-glyph. Nothing scrolls horizontally, so none of it is reachable. **Two causes were tested and refuted, both by rebuilding and re-measuring: removing `viewport-width: self.width` changed nothing, and removing `width: parent.width` from the content plus all four layout branches changed nothing.** Next: place the same marker outside the `ScrollView` as a sibling, to find whether the doubling happens at the scroll boundary or above it. | the `ScrollView` in `DashboardView`, `crates/desktop/ui/views/dashboard-view.slint` |
