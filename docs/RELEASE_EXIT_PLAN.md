...
| **P3** | **Cards are clipped at the right edge instead of fitting or scrolling.** In the same capture the Today card's `TOKENS` column is cut mid-glyph at the window border, and `Code Output` loses its day/week toggle the same way. Nothing scrolls horizontally, so the content is simply unreachable. Seen at both 200% and 100% scale, so it is not a DPI artefact. | the Today strip and `CodeOutputCard` in `dashboard-view.slint` |
