...
| **P3** | **A row of three empty cells at stretch 2:1:1 gives the first one the entire visible width.** The Today row's three cells were replaced with three bare `Rectangle`s carrying only a colour and the real stretch -- no text, no wrappers, nothing that could distort a preferred width. The first cell runs from the card's left edge to the window border and the other two never render, at every window width tried and **also at the size the application chooses for itself**, which rules out the external `MoveWindow` used to resize during measurement. The row is therefore laid out at roughly twice the width it occupies, and the two lost metrics are unreachable rather than off-screen, since nothing scrolls horizontally. **Ruled out by experiment, each with a rebuild and a re-measure:** `viewport-width: self.width` on the board's `ScrollView`; `width: parent.width` on the scrolled content and all four layout branches; `width: parent.width` on the shell's own root layout. Next: bisect by placing the same three-cell probe at each level between the window root and the card, giving it an explicit `width` -- a `HorizontalLayout` of stretch-only children has a preferred width of zero and renders as nothing when a non-layout parent does not size it. | `DashboardHeaderCard` and the shell in `crates/desktop/ui/main.slint` |
