...
| Phase | Item | Anchor |
|---|---|---|
| **P3 exit** | **No lifetime total exists, and the phase cannot close without one.** The criterion is in the Sequence table but was never tracked as an item. Established by reading: `UsageStore::counts` returns row counts only -- sources, generations, observations, events, chunks, scans -- and no token or cost sum exists in the store, the query layer or any projection. The query layer cannot express it either: every range runs through `UsageRange`, and `custom` rejects a span over `MAX_QUERY_SERIES_POINTS = 400` days, so a lifetime is not a long range. The `Total tokens` already on the History view is the total of the selected range, not of the archive. Shape: an unbounded aggregate in the store, a request that carries no dates, then projection and a place to show it. | `fn counts` in `crates/store/src/usage/read.rs`; `fn custom` in `crates/query/src/analytics.rs` |
