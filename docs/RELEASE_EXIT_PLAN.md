...
- **Whether to dispatch the release workflow once, to prove it can build.** It has never
  ...
  one, so the run can only happen after the merge above. The workflow holds
  `attestations: write`, and an attestation is a public record -- which is the whole reason
  this is written down as a question rather than done.
