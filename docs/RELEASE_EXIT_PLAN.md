...
| **P3** | **Cards are clipped at the right edge instead of fitting or scrolling.** In the same capture the Today card's `TOKENS` column is cut mid-glyph at the window border, and `Code Output` loses its day/week toggle the same way. Nothing scrolls horizontally **by construction**: the board's `ScrollView` declares `viewport-width: self.width`, so the viewport can never exceed the visible width and anything past it is unreachable rather than merely off-screen. No `min-width` is declared anywhere in the file, so what pushes the row over is a natural width, not a floor. Seen at both 200% and 100% scale, so it is not a DPI artefact. | the Today strip and `CodeOutputCard` in `dashboard-view.slint` |
