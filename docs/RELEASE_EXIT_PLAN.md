...
| **P3** | **A card with no data claims the height of a card full of it, and the board is one column.** Captured at `SLINT_SCALE_FACTOR=1` in a 1900x1400 window: `Plan Usage` renders one line, `Quota evidence unavailable`, inside a box about 500 px tall, and `Code Output` spreads four numbers over a similar span. The six sections stack vertically at this width, so the trend card sits below all of them and cannot be brought on screen at any window size this display can hold -- which is also why the trend's own area fill and legend are still visually unconfirmed. `min-height` already drops to 96px for a section without data, so the height is coming from the layout stretching slots to fill, not from the minimum. | the `min-height` expression on `DashboardBoardSlot` and the enclosing layout in `dashboard-view.slint` |
| **P3 blocker** | **The window renders at scale 2 while every size it asks for is applied at scale 1, so it opens at half the logical width the interface is drawn for.** Measured on a 200% display: the sidebar declares `168px` and occupies 332 physical pixels, a ratio of 1.976. The window opens at 1061x796 physical against a declared `preferred-width: 1048px`, which makes the effective logical width about 566. `DashboardView` then gets roughly 392 logical against its own `narrow` threshold of 944px, so the wide two-column layout is unreachable at any size the application chooses, the board always stacks in one column, and the Today row never fits three metrics -- `EVENTS` does not render at all and `TOKENS` is cut at the border. `scale_factor()` reports 1.0, proven by `set_size(PhysicalSize(width * scale_factor()))` producing exactly `width` physical pixels, so a fix cannot use it; the real factor has to come from the monitor, `GetDpiForWindow` on Windows, which belongs in `platform` rather than the UI crate. **Refuted along the way, each by rebuild and re-measure:** that `root.width` is doubled (a `100px` bar renders 200 physical and `root.width - 200px` is consistent with the true width), and that `horizontal-stretch` misbehaves (changing 2:1:1 to 1:1:1 moved `TOKENS` left by exactly the expected amount). | `preferred-width` in `crates/desktop/ui/main.slint`; window sizing in `crates/desktop/src/ui.rs` |
| note | **Measuring a Slint layout from a screenshot is where this investigation keeps failing, not the diagnosis.** Four instruments gave false readings before any of them gave a usable one. A full-width marker cannot tell "fits" from "clipped" -- both end on the last pixel. A marker built from `horizontal-stretch` reported the same impossible doubling at two independent levels, which is far likelier to be the instrument than two identical bugs. A yellow marker collided with the gold `TokenMaster` wordmark. A marker inserted before the shell was hidden behind its opaque background. What a usable instrument needs: a colour the product never uses, a position anchored absolutely rather than through a layout, placement last in source order so it draws on top, and a visible right end. Also: `PrintWindow` renders nothing beyond the physical screen, in either axis, so a window cannot be grown past 2880 px to measure what overflows it. | measurement technique |
