...
| **P3** | **The Today row gives one cell nearly the whole width, so two metrics are unreachable.** Measured at the display's own scale, with the three cells temporarily given distinct backgrounds: at a 1400 px window the `COST` cell spans 869 px and at 2400 px it spans 1869 px -- about 88% and 94% of the row -- while `TOKENS` is cut at the window border and `EVENTS` never renders at any size. The declared stretch is 2:1:1, which would give `COST` half. Nothing scrolls horizontally, so the two lost metrics are not merely off-screen, they are unreachable. **Not yet explained.** Three independent instruments agree the row is laid out at roughly twice the width it occupies, but the wrappers needed to colour the cells themselves take part in the layout, so the instrument alters what it measures. **Stop measuring through screenshots**: the repository already carries the non-default `live-preview` feature, which shows the computed layout directly and would answer this in one session instead of five rebuilds. | `DashboardHeaderCard` in `crates/desktop/ui/views/dashboard-view.slint`; enable `live-preview` |
