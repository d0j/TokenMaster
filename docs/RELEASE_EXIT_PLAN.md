...
| post-tag | `hide()` → `show()` was suspected of skipping the partial-render cache invalidation that minimize → restore gets, because winit emits no `Occluded` on Windows. **Not reproduced.** Driven on a 200% display by posting the tray icon's own click message, the window hid for fifteen seconds with `IsWindowVisible` false and `IsIconic` false — the exact `hide()` path — and restored with a screen capture of the presented surface indistinguishable from the one taken before hiding: 105 versus 104 distinct sampled colours, mean luma 32.4 versus 32.3. The mechanism in `docs/SLINT_NOTES.md` is still real; what is not established is that it produces a visible stale window here. Reopen only with a capture that shows one. | one machine, one build, one duration |
