...
use super::git_schema::{
    V13_GIT_INDEX_CONTRACTS, V13_GIT_SCHEMA, V13_GIT_TABLE_CONTRACTS, V13_GIT_TRIGGER_CONTRACTS,
};
...
pub(super) fn migrate_schema(connection: &mut Connection) -> Result<(), StoreError> {
    ...
    match version {
        V2_SCHEMA_VERSION => {
            ...
            migrate_v10_then_current(connection)
        }
        V3_SCHEMA_VERSION => {
...
fn validate_extended_schema(
    ...
    if include_git {
        trigger_contracts.extend(V13_GIT_TRIGGER_CONTRACTS.iter().copied());
    }
    ...
    if include_git {
        schema_sources.push(V13_GIT_SCHEMA);
        extension_tables.extend_from_slice(V13_GIT_TABLE_CONTRACTS);
    }
    validate_schema(
        connection,
        USAGE_TABLE_CONTRACTS,
...
fn validate_git_state(connection: &Connection) -> Result<(), StoreError> {
    let valid = connection
        .query_row(
            "SELECT length(installation_salt) = 32
                    AND publication_revision >= 0
                    AND repository_count = (SELECT count(*) FROM git_repository)
                    AND association_count = (SELECT count(*) FROM git_activity_association)
                    AND repository_count BETWEEN 0 AND 32
                    AND association_count BETWEEN 0 AND 4096
                    AND ((publication_revision = 0 AND last_published_at_ms IS NULL)
                      OR (publication_revision > 0 AND last_published_at_ms IS NOT NULL))
                    AND NOT EXISTS(
                      SELECT 1 FROM git_day_aggregate AS item
                      JOIN git_repository AS repository USING(repository_id)
                      WHERE item.aggregate_generation <> repository.active_generation
                    )
                    AND NOT EXISTS(
                      SELECT 1 FROM git_day_category_aggregate AS item
                      JOIN git_repository AS repository USING(repository_id)
                      WHERE item.aggregate_generation <> repository.active_generation
                    )
                    AND NOT EXISTS(
                      SELECT 1 FROM git_category_aggregate AS item
                      JOIN git_repository AS repository USING(repository_id)
                      WHERE item.aggregate_generation <> repository.active_generation
                    )
                    AND NOT EXISTS(
                      SELECT 1 FROM git_warning AS item
                      JOIN git_repository AS repository USING(repository_id)
                      WHERE item.aggregate_generation <> repository.active_generation
                    )
                    AND NOT EXISTS(
                      SELECT 1
                      FROM git_repository AS repository
                      WHERE NOT EXISTS(
                              SELECT 1 FROM git_activity_association AS association
                              WHERE association.repository_id = repository.repository_id
                            )
                         OR (repository.quality = 'unavailable' AND (
                              EXISTS(SELECT 1 FROM git_day_aggregate AS day
                                     WHERE day.repository_id = repository.repository_id)
                           OR EXISTS(SELECT 1 FROM git_day_category_aggregate AS category
                                     WHERE category.repository_id = repository.repository_id)
                           OR EXISTS(SELECT 1 FROM git_category_aggregate AS category
                                     WHERE category.repository_id = repository.repository_id)
                           OR EXISTS(SELECT 1 FROM git_warning AS warning
                                     WHERE warning.repository_id = repository.repository_id)
                         ))
                         OR (repository.quality <> 'unavailable' AND (
                              (SELECT count(*) FROM git_day_aggregate AS day
                               WHERE day.repository_id = repository.repository_id
                                 AND day.aggregate_generation = repository.active_generation) > 400
                           OR (SELECT coalesce(sum(commits), 0)
                               FROM git_day_aggregate AS day
                               WHERE day.repository_id = repository.repository_id
                                 AND day.aggregate_generation = repository.active_generation)
                              > repository.commits
                           OR (SELECT coalesce(sum(merge_commits), 0)
                               FROM git_day_aggregate AS day
                               WHERE day.repository_id = repository.repository_id
                                 AND day.aggregate_generation = repository.active_generation)
                              > repository.merge_commits
                           OR (SELECT coalesce(sum(added_lines), 0)
                               FROM git_day_aggregate AS day
                               WHERE day.repository_id = repository.repository_id
                                 AND day.aggregate_generation = repository.active_generation)
                              > repository.added_lines
                           OR (SELECT coalesce(sum(removed_lines), 0)
                               FROM git_day_aggregate AS day
                               WHERE day.repository_id = repository.repository_id
                                 AND day.aggregate_generation = repository.active_generation)
                              > repository.removed_lines
                           OR (EXISTS(
                                SELECT 1 FROM git_warning AS warning
                                WHERE warning.repository_id = repository.repository_id
                                  AND warning.aggregate_generation = repository.active_generation
                                  AND warning.warning = 'daily_history_truncated'
                              ) <> (
                           ...
                           OR (SELECT count(*) FROM git_category_aggregate AS category
                               WHERE category.repository_id = repository.repository_id
                                 AND category.aggregate_generation = repository.active_generation) <> 8
                           OR (SELECT count(*) FROM git_day_category_aggregate AS category
                               WHERE category.repository_id = repository.repository_id
                                 AND category.aggregate_generation = repository.active_generation)
                              <> 8 * (SELECT count(*) FROM git_day_aggregate AS day
                                      WHERE day.repository_id = repository.repository_id
                                        AND day.aggregate_generation = repository.active_generation)
                           OR (SELECT coalesce(sum(added_lines), 0)
                               FROM git_category_aggregate AS category
                               WHERE category.repository_id = repository.repository_id
                                 AND category.aggregate_generation = repository.active_generation)
                              <> repository.added_lines
                           OR (SELECT coalesce(sum(removed_lines), 0)
                           ...
                           OR EXISTS(
                                ...
                                WHERE day.repository_id = repository.repository_id
                                  AND day.aggregate_generation = repository.active_generation
                                  AND (
                                    (SELECT count(*) FROM git_day_category_aggregate AS category
                                     WHERE category.repository_id = day.repository_id
                                       AND category.aggregate_generation = day.aggregate_generation
                                       AND category.day_index = day.day_index) <> 8
                                    OR (SELECT coalesce(sum(added_lines), 0)
                                        FROM git_day_category_aggregate AS category
                                        WHERE category.repository_id = day.repository_id
                                          AND category.aggregate_generation = day.aggregate_generation
                                          AND category.day_index = day.day_index) <> day.added_lines
                                    OR (SELECT coalesce(sum(removed_lines), 0)
                                        FROM git_day_category_aggregate AS category
                                        WHERE category.repository_id = day.repository_id
                                          AND category.aggregate_generation = day.aggregate_generation
                                          AND category.day_index = day.day_index) <> day.removed_lines
                                  )
                              )
                         ))
                         OR (SELECT count(*) FROM git_warning AS warning
                             WHERE warning.repository_id = repository.repository_id
                               AND warning.aggregate_generation = repository.active_generation) > 16
                         OR (repository.quality = 'complete' AND EXISTS(
                              SELECT 1 FROM git_warning AS warning
                              WHERE warning.repository_id = repository.repository_id
                                AND warning.aggregate_generation = repository.active_generation
                            ))
                         OR (repository.quality = 'partial' AND NOT EXISTS(
                              SELECT 1 FROM git_warning AS warning
                              WHERE warning.repository_id = repository.repository_id
                                AND warning.aggregate_generation = repository.active_generation
                            ))
                         OR ((repository.binary_files > 0) <> EXISTS(
                              SELECT 1 FROM git_warning AS warning
                              WHERE warning.repository_id = repository.repository_id
                                AND warning.aggregate_generation = repository.active_generation
                                AND warning.warning = 'binary_files_omitted'
                            ))
                         OR ((repository.submodule_changes > 0) <> EXISTS(
                              SELECT 1 FROM git_warning AS warning
                              WHERE warning.repository_id = repository.repository_id
                                AND warning.aggregate_generation = repository.active_generation
                                AND warning.warning = 'submodule_lines_omitted'
                            ))
                         OR ((repository.omitted_paths > 0) <> EXISTS(
                              SELECT 1 FROM git_warning AS warning
                              WHERE warning.repository_id = repository.repository_id
                                AND warning.aggregate_generation = repository.active_generation
                                AND warning.warning = 'oversized_fields_omitted'
                            ))
                         OR ((repository.omitted_commits > 0) <> EXISTS(
                              SELECT 1 FROM git_warning AS warning
                              WHERE warning.repository_id = repository.repository_id
                                AND warning.aggregate_generation = repository.active_generation
                                AND warning.warning = 'invalid_commit_omitted'
                            ))
                         OR ((repository.shallow = 1) <> EXISTS(
                              SELECT 1 FROM git_warning AS warning
                              WHERE warning.repository_id = repository.repository_id
                                AND warning.aggregate_generation = repository.active_generation
                                AND warning.warning = 'shallow_history'
                            ))
                    )
...
fn migrate_v12(connection: &mut Connection) -> Result<(), StoreError> {
...
fn migrate_v11_with_fault(
    ...
    transaction.pragma_update(None, "user_version", V12_SCHEMA_VERSION)?;
...
mod tests {
    ...
    #[test]
    fn git_schema_fault_rolls_back_exact_v12_without_residue() -> TestResult {
        let mut connection = exact_v9_fixture()?;
        migrate_v9_with_fault(&mut connection, MigrationFault::None)?;
        migrate_v10_with_fault(&mut connection, MigrationFault::None)?;
        ...
        let error =
            match migrate_v12_with_fault(&mut connection, MigrationFault::AfterCreateGitSchema) {
                Ok(()) => return Err("faulted Git schema migration committed".into()),
                Err(error) => error,
            };
        ...
        assert_eq!(
            pragma_i64(
                &connection,
                "SELECT count(*) FROM sqlite_schema WHERE name LIKE 'git_%'"
            )?,
            0
        );
        validate_v12(&connection)?;
