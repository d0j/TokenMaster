...
const COMPONENT_MSGIDS: [&str; 33] = [
    // The section-state chip presents these; the state key itself stays untranslated
    // so Rust keeps one stable contract and only the presentation is localized.
    "Ready",
    "Degraded",
    "Waiting",
    "Unavailable",
...
const SESSIONS_DASHBOARD_MSGIDS: [&str; 51] = [
    "Sessions {0} {1}",
    "Sessions",
...
const PROJECTION_MSGIDS: [&str; 96] = [
    "{0} · {1}",
    "{0} {1} capacity",
    "{0} {1} remaining",
    "{0} {1} used",
    ...
    "Loading",
    "Ready",
    "Ready · breakdown limited to 32 models and 32 projects",
    "Not found",
    "Unavailable · {0}",
];
...
const NOTIFICATIONS_PROJECTION_MSGIDS: [&str; 55] = [
    "Waiting",
    "Inventory unavailable",
    "Bounded inventory · more data omitted",
    "Current inventory · warnings present",
    "Current inventory complete",
    "{0} reminder profile · {1} current benefit",
    "{0} reminder profile · {1} current benefits",
    "{0} reminder profiles · {1} current benefit",
    "{0} reminder profiles · {1} current benefits",
    "Scope {0}",
    "{0} benefit",
    "{0} benefits",
    "In-app only",
    ...
    "Inherited",
    "Override",
    "Complete quantity, partial details",
    "Next reminder unavailable",
    "Next reminder {0}",
    "Nearest expiry unavailable",
    "Nearest expiry {0}",
    ...
    "Ambiguous",
    "Expired {0}",
    "Expired between {0} and {1}",
    "Expires between {0} and {1}",
    "Expired {0} (provider local)",
    "Expires {0} (provider local)",
    "Expired {0} (provider date)",
    "Expires {0} (provider date)",
    "Expiry unknown",
    "Granted {0}",
...
fn sessions_and_dashboard_catalogs_are_complete_before_view_conversion() {
    let expected = SHELL_MSGIDS
        ...
        .chain(IN_APP_NOTIFICATION_PROJECTION_MSGIDS)
        .collect::<BTreeSet<_>>();
    assert_eq!(expected.len(), 517, "projection catalog exact inventory");
    ...
    for locale in ["ru", "pseudo"] {
        ...
        assert_eq!(po_entry_count(&catalog), 517, "{locale} exact key count");
...
#[test]
fn locale_switch_localizes_session_detail_status_atoms_without_mutating_unknown_codes() {
    let _locale_guard = LOCALE_SWITCH_LOCK.lock().expect("locale switch lock");
    i_slint_backend_testing::init_no_event_loop();
    let sink = Rc::new(RecordingIntentSink {
        selection: Cell::new(None),
    ...
    assert_eq!(
        strings.invoke_session_detail_status_label("ready".into(), false),
        "Ready"
    );
    assert_eq!(
        strings.invoke_session_detail_unavailable_label("Request Rejected".into()),
        "Unavailable · Request Rejected"
    );
    assert_eq!(
        strings.invoke_session_detail_status_label("provider_state".into(), false),
        "provider_state"
    );

    window.invoke_select_presentation_locale(1);
    for (code, truncated, expected) in [
        ("idle", false, "Нет выбора"),
        ("loading", false, "Загрузка"),
        ("ready", false, "Готово"),
        (
            "ready",
            true,
            "Готово · разбивка ограничена 32 моделями и 32 проектами",
        ),
        ("missing", false, "Не найдено"),
        ("unavailable", false, "Недоступно"),
    ] {
        assert_eq!(
            strings.invoke_session_detail_status_label(code.into(), truncated),
            expected
        );
    }
    assert_eq!(
        strings.invoke_session_detail_unavailable_label("Request Rejected".into()),
        "Недоступно · Request Rejected",
        "the existing humanized failure suffix remains unchanged"
    );
    assert_eq!(
        strings.invoke_session_detail_status_label("provider_state".into(), false),
        "provider_state"
...
#[test]
fn locale_switch_localizes_closed_notifications_projection_atoms_without_mutating_unknown_codes() {
    let _locale_guard = LOCALE_SWITCH_LOCK.lock().expect("locale switch lock");
    i_slint_backend_testing::init_no_event_loop();
    let sink = Rc::new(RecordingIntentSink {
        selection: Cell::new(None),
    ...
    assert_eq!(
        strings.invoke_notification_kind_label("banked_rate_limit_reset".into()),
        "Banked rate limit reset"
    );
    assert_eq!(
        strings.invoke_notification_kind_label("provider_custom_kind".into()),
        "provider_custom_kind"
    );
    ...
    assert_eq!(
        strings.invoke_notification_warning_label("unknown_expiry".into()),
...
fn shell_and_presentation_strip_use_only_the_closed_translation_key_set() {
    ...
    for msgid in NOTIFICATIONS_PROJECTION_MSGIDS {
        assert!(
            projection_strings.contains(&format!("@tr(\"{msgid}\"")),
