...
const SESSIONS_DASHBOARD_MSGIDS: [&str; 50] = [
    "Sessions {0} {1}",
    "Sessions",
...
fn sessions_and_dashboard_catalogs_are_complete_before_view_conversion() {
    let expected = SHELL_MSGIDS
        ...
        .chain(IN_APP_NOTIFICATION_PROJECTION_MSGIDS)
        .collect::<BTreeSet<_>>();
    assert_eq!(expected.len(), 511, "projection catalog exact inventory");
    ...
    for locale in ["ru", "pseudo"] {
        ...
        assert_eq!(po_entry_count(&catalog), 511, "{locale} exact key count");
